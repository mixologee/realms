...
int save_objects(zone_rnum zone_num)
{
  ...
  long int counter, counter2, realcounter;
  FILE *fp;
  struct obj_data *obj;
  ...
  for (counter = genolc_zone_bottom(zone_num); counter <= zone_table[zone_num].top; counter++) {
	...
	*buf = '\0';
	  ...
	  GET_OBJ_TYPE(obj),
          ...
          GET_OBJ_COND(obj), GET_OBJ_LEVEL(obj)
      );
...
void restring_object(struct obj_data *obj, int rs_pos, char *new_string)
{
  ...
  obj_rnum robj_num = GET_OBJ_RNUM(obj);

  smash_tilde(new_string);

  switch(rs_pos) {
    case RS_OBJ_NAME: //obj name
       if ((robj_num == NOTHING) || (obj->name && obj->name != obj_proto[robj_num].name))
         free(obj->name);
         obj->name = strdup(new_string);
       break;
    case RS_OBJ_DESC:  // obj description
       if ((robj_num == NOTHING) || (obj->description && obj->description != obj_proto[robj_num].description))
         free(obj->description);
         obj->description = strdup(new_string);
       break;
    case RS_OBJ_ADESC: // obj short description
       if ((robj_num == NOTHING) || (obj->short_description && obj->short_description != obj_proto[robj_num].short_description))
         free(obj->short_description);
         obj->short_description = strdup(new_string);
       break;
    case RS_OBJ_SDESC:  // obj action description
       if ((robj_num == NOTHING) || (obj->action_description && obj->action_description != obj_proto[robj_num].action_description))
         free(obj->action_description);
         obj->action_description = strdup(new_string);
       break;
    default:
      log("SYSERR: Invalid restring type (%d) passed to restring_object", rs_pos);
      nmudlog(SYS_LOG, LVL_GOD, TRUE, "SYSERR: invalid string type (%d) passed to restring_object", rs_pos);
      break;
  }
}
...
void restring_mobile(struct char_data *mob, int rs_pos, char *new_string)
{
  ...
  mob_rnum rmob_num = GET_MOB_RNUM(mob);
  ...
  switch(rs_pos) {
     ...
     case RS_MOB_NAME:
        if ((rmob_num == NOTHING) || (mob->player.name && mob->player.name != mob_proto[rmob_num].player.name))
          free(mob->player.name);
          mob->player.name = strdup(new_string);
        break;
     case RS_MOB_DESC:
        if ((rmob_num == NOTHING) || (mob->player.description && mob->player.description != mob_proto[rmob_num].player.description))
          free(mob->player.description);
          mob->player.description = strdup(new_string);
        break;
     case RS_MOB_SDESC:
        if ((rmob_num == NOTHING) || (mob->player.short_descr && mob->player.short_descr != mob_proto[rmob_num].player.short_descr))
          free(mob->player.short_descr);
          mob->player.short_descr = strdup(new_string);
        break;
     case RS_MOB_LDESC:
        if ((rmob_num == NOTHING) || (mob->player.long_descr && mob->player.long_descr != mob_proto[rmob_num].player.long_descr))
          free(mob->player.long_descr);
          mob->player.long_descr = strdup(new_string);
        break;
     default:
        log("SYSERR: Invalid restring type (%d) passed to restring_mobile", rs_pos);
        nmudlog(SYS_LOG, LVL_GOD, TRUE, "SYSERR: invalid restring type (%d) passed to restring_mobile", rs_pos);
        break;
...
void restring(void *data, int rs_pos, char *new_string)
{
  switch(rs_pos) {
     ...
     case RS_OBJ_SDESC:
     case RS_OBJ_ADESC:
        restring_object((struct obj_data *)data, rs_pos, new_string);
        break;
     case RS_MOB_NAME:
     case RS_MOB_DESC:
     case RS_MOB_SDESC:
     case RS_MOB_LDESC:
        restring_mobile((struct char_data *)data, rs_pos, new_string);
        break;
     default:
        log("SYSERR: Invalid restring type (%d) passed to reststring (genobj.c)", rs_pos);
        nmudlog(SYS_LOG, LVL_GOD, TRUE, "SYSERR: invalid restring type (%d) passed to resting (genobj.c)", rs_pos);
        break;
  }
}
