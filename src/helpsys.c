...
void hedit_disp_type_menu(struct descriptor_data * d);
...
extern const char *help_category[];
...
void hedit_disp_menu(struct descriptor_data * d)
{
  ...
  write_to_output(d,
    ...
    help_category[OLC_HELP(d)->category]);
...
void hedit_disp_type_menu(struct descriptor_data * d)
{
  ...
  write_to_output(d,
    "\r\n&WPlease choose the category of this help file\r\n"
    "&W1) &cCommands\r\n" 
    "&W2) &cInfo\r\n"
    "&W3) &cOasis\r\n"
    "&W4) &cSkills\r\n"
  ...
  case HEDIT_CONFIRM_SAVE:
    ...
    case HEDIT_MAIN_MENU:
      ...
      case '4':
        ...
        break;
      case '5':
        hedit_disp_type_menu(d);
        break;
      case 'p':
      case 'P':
        if (GET_LEVEL(d->character) >= LVL_IMPL) {
          newhelp = OLC_HELP(d)->index;  /* next new help will get this one's number  */
          /* free everything up, including strings etc */
...
void hedit_setup_existing(struct descriptor_data *d, int number)
{
  ...
  fprintf(stderr, "hnum = %d   helpnum = %d  newhelp = %d\r\n", hnum, helpnum, newhelp);
}
...
void load_help(void)
{
  ...
  for (;;) {
    ...
    if ((hptr = enqueue_help()) != NULL) {
      ...
      hptr->category = atoi(fread_string(fl, buf));
...
void save_help(void)
{
  ...
  fprintf(stderr, "Saving help file.\n");
...
{
  char buf[MAX_STRING_LENGTH], buf2[MAX_STRING_LENGTH], arg[MAX_INPUT_LENGTH];
  int i = 0, cat=10;
  ...
  *buf2 = '\0';
  ...
  if(!*argument)
  {
   send_to_char(ch, "&RWhat help category do you want the keywords for?&n\r\n\r\n");
   send_to_char(ch, " &WCommands   Info    Oasis\r\n"
                    " Skills     Spells  Socials\r\n"
		    " Subjects   Wizhelp Help&n\r\n");
   return;
  }

  if (is_abbrev(arg, "commands")) 
    cat = 1;
  else if (is_abbrev(arg, "info"))
    cat = 2;
  else if (is_abbrev(arg, "oasis"))
    cat = 3;
  else if (is_abbrev(arg, "skills"))
    cat = 4;
  else if (is_abbrev(arg, "spells"))
    cat = 5;
  else if (is_abbrev(arg, "socials"))
    cat = 6;
  else if (is_abbrev(arg, "subjects"))
    cat = 7;
  else if (is_abbrev(arg, "wizhelp"))
    cat = 8;
  else if (is_abbrev(arg, "help"))
    cat = 9;
  else {
    send_to_char(ch, "Invalid Choice. Try again.\r\n");
    return;
  }
 
  snprintf(buf2, sizeof(buf2), "Category       Help#      Keyword\r\n");
  strncat(buf, buf2, sizeof(buf));
  snprintf(buf2, sizeof(buf2), "---------------------------------\r\n");
  strncat(buf, buf2, sizeof(buf));
 
  for (hptr = keyword_list; hptr; hptr = hptr->next) {
    if(hptr->category == cat) {
      snprintf(buf2, sizeof(buf2), "%s            %d       %s\r\n", help_category[cat], hptr->helpid, hptr->keyword);
      strncat(buf, buf2, sizeof(buf));
    }
  }
