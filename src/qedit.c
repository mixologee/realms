...
ACMD(do_oasis_qedit)
{
  ...
  if (save) {
    send_to_char(ch, "Saving all quests in zone %ld.\r\n",
      zone_table[OLC_ZNUM(d)].number);
    mudlog(CMP, MAX(LVL_BUILDER, GET_INVIS_LEV(ch)), TRUE,
      "OLC: %s saves quest info for zone %ld.",
  ...
  mudlog(BRF, LVL_IMMORT, TRUE,
         "OLC: %s starts editing zone %ld allowed zone %d",
...
void qedit_setup_new(struct descriptor_data *d)
{
  ...
  quest->value[0]   = 1;              /* Points for completing  */
  quest->value[1]   = 1;              /* Points for abandoning  */
  quest->value[2]   = 1;              /* Minimum level          */
  quest->value[3]   = LVL_IMPL;       /* Maximim level          */
  quest->value[4]   = 1;              /* Time limit             */
  quest->value[5]   = 1;         /* Mob to return object   */
  quest->value[6]   = 1;              /* Quantity of targets    */
  quest->prev_quest = NOTHING;        /* Previous quest         */
  quest->next_quest = NOTHING;        /* Next quest             */
  quest->gold_reward= 1;              /* Prize in gold coins    */
  quest->exp_reward = 1;              /* Prize in exp points    */
...
void qedit_disp_menu(struct descriptor_data *d)
{
  ...
  write_to_output(d,
    ...
    "&g 8&n) Quest Master   : [&c%6ld&n] &y%s\r\n"
    "&g 9&n) Quest Target   : [&c%6ld&n] &y%s\r\n"
    "&g A&n) Quantity       : [&c%6d&n]\r\n"
    "&n    Quest Point Rewards\r\n"
    "&g B&n) Completed      : [&c%6d&n] &g C&n) Abandoned   : [&c%6d&n]\r\n"
    "&n    Other Rewards Rewards\r\n"
    "&g G&n) Gold Coins     : [&c%6d&n] &g T&n) Exp Points  : [&c%6d&n] &g O&n) Object : [&c%6ld&n]\r\n"
    "&n    Level Limits to Accept Quest\r\n"
    "&g D&n) Lower Level    : [&c%6d&n] &g E&n) Upper Level : [&c%6d&n]\r\n"
    "&g F&n) Prerequisite   : [&c%6ld&n] &y%s\r\n"
    "&g L&n) Time Limit     : [&c%6d&n]\r\n"
    "&g N&n) Next Quest     : [&c%6ld&n] &y%s\r\n"
    "&g P&n) Previous Quest : [&c%6ld&n] &y%s\r\n"
    "&g X&n) Delete Quest\r\n"
    "&g Q&n) Quit\r\n"
    "Enter Choice : ",
    quest->vnum,
  ...
  switch (OLC_MODE(d)) {
    ...
    case QEDIT_CONFIRM_SAVESTRING:
      switch (*arg) {
	    ...
	    "OLC: %s edits quest %ld", GET_NAME(d->character), OLC_NUM(d));
          if (CONFIG_OLC_SAVE) {
            qedit_save_to_disk(real_zone_by_thing(OLC_NUM(d)));
            write_to_output(d, "Quest %ld saved to disk.\r\n", OLC_NUM(d));
          } else
            write_to_output(d, "Quest %ld saved to memory.\r\n", OLC_NUM(d));
