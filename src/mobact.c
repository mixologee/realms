...
void mobile_activity(void)
{
  ...
  for (ch = character_list; ch; ch = next_ch) {
    ...
    /* Mob Movement */
    if (!AFF_FLAGGED(ch, AFF_GROUP) && !MOB_FLAGGED(ch, MOB_SENTINEL) && (GET_POS(ch) == POS_STANDING) &&
    ...
    if (OUTSIDE(ch) && MOB_FLAGGED(ch, MOB_DAY_ONLY) && Sunlight == SUN_DARK) {
       act("$n, being scared of the dark, runs off to find some shelter.", FALSE, ch, 0, 0, TO_ROOM);
       extract_mobile_all(GET_MOB_VNUM(ch)); 
       continue;
    }

    /* Night Only Mobs */

    if (OUTSIDE(ch) && MOB_FLAGGED(ch, MOB_NIGHT_ONLY) && Sunlight == SUN_LIGHT) {
       act("$n flees from the sunlight into a darker area.", FALSE, ch, 0, 0, TO_ROOM);
       extract_mobile_all(GET_MOB_VNUM(ch));
       continue;
    }
	...
	for (names = MEMORY(ch); names && !found; names = names->next) {
	  if (names->id != GET_IDNUM(vict))
          ...
          if (!IS_NPC(vict) && CONFIG_SAFE_MAILING && PLR_FLAGGED(vict, PLR_MAILING))
           continue;
