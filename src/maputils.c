...
//  y = world[room].y;
  ...
  for (i=0; i<=top_of_world; i++)
  {
     ...
     if (world[i].number == z-1)
       w=TRUE;
/*
     if (world[room].dir_option[NORTH] != NULL)
       s=TRUE;
     if (world[room].dir_option[SOUTH] != NULL)
       n=TRUE;
     if (world[room].dir_option[EAST] != NULL)
       w=TRUE;
     if (world[room].dir_option[WEST] != NULL)
       e=TRUE;
*/
  }
...
/* MAKE ROOM NORTH */
   if (from != NORTH && n==FALSE) {
      ...
      for (t = 0; MapSymbolTable[t].Symbol != 'Q'; t++) {       /* Compare with Symbol Table*/
        if (MapSymbolTable[t].Symbol == Wildneress[y + 1][x] && MapSymbolTable[t].red == Pixels[y+1][x].r &&
            MapSymbolTable[t].green == Pixels[y+1][x].g && MapSymbolTable[t].blue == Pixels[y+1][x].b){ /* Match Found break*/
     ...
     for(rn=0; rn < top_of_world;rn++) {
      ...
      if (world[rn].dir_option[SOUTH] == NULL) {
...
/* MAKE ROOM SOUTH */
    if (from != SOUTH && s==FALSE) {
     ...
     for(rs=0; rs < top_of_world;rs++) {
      ...
      if (world[rs].dir_option[NORTH] == NULL) {
...
/* MAKE ROOM EAST  */
    if (from != EAST && e==FALSE) {
     ...
     for(re=0; re < top_of_world;re++) {
      ...
      if (world[re].dir_option[WEST] == NULL) {
...
/* MAKE ROOM WEST  */
    if (from != WEST && w==FALSE) {
     ...
     for(rw=0; rw < top_of_world;rw++) {
      ...
      if (world[rn].dir_option[EAST] == NULL) {
