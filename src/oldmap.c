...
/* Crystal Gates */
	...
	obj_to_room(obj, real_room(world[top_of_world].number));  
 ...
 *             room with using the specified information.
 *             */
void MakeWildRoom(CHAR_DATA *ch)
{
  int y, x, i, rf;      /* Coordinates for Wildneress */
  long int z;   /* Room Number*/
  int t;        /* Search Block Location*/
...
// FUNCTION: LinkMapRooms
// USAGE   : From the actual information created from the interpertation
//           this function will link north/south east/west dirst and create
//           the actual map that is used int the game.
void LinkMapRooms(void)
{
  ...
  for (y = MAPY - 1, z = 1 ; y >= 0 ; y--) {
    for (x = 0; x < MAPX ; x++, z++) {
      a = top_of_world - (MAPY * MAPX) + z;
      b = top_of_world - (MAPY * MAPX) + z + MAPY;
      c = top_of_world - (MAPY * MAPX) + z + 1;

      if (y) {
        CREATE(world[a].dir_option[NORTH], struct room_direction_data, 1);
        CREATE(world[b].dir_option[SOUTH], struct room_direction_data, 1);

        world[a].dir_option[NORTH]->general_description = NULL;
        world[a].dir_option[NORTH]->keyword = NULL;
        world[a].dir_option[NORTH]->to_room = b;
        world[a].dir_option[NORTH]->exit_info = 0;

        world[b].dir_option[SOUTH]->general_description = NULL;
        world[b].dir_option[SOUTH]->keyword = NULL;
        world[b].dir_option[SOUTH]->to_room = a;
        world[b].dir_option[SOUTH]->exit_info = 0;
      }
      if (x > 1998) {
        continue;
      } else {
        CREATE(world[a].dir_option[EAST], struct room_direction_data, 1);
        CREATE(world[c].dir_option[WEST], struct room_direction_data, 1);

        world[a].dir_option[EAST]->general_description = NULL;
        world[a].dir_option[EAST]->keyword = NULL;
        world[a].dir_option[EAST]->to_room = c;
        world[a].dir_option[EAST]->exit_info = 0;

        world[c].dir_option[WEST]->general_description = NULL;
        world[c].dir_option[WEST]->keyword = NULL;
        world[c].dir_option[WEST]->to_room = a;
        world[c].dir_option[WEST]->exit_info = 0;
      }
    }
  }
  log("linked rooms");
...
int MapCreateDir(int room, int dir)
{
  if ((room > top_of_world) || (room < 0)) {
    log("MapCreateDir(): tried to create invalid door");
    return FALSE;
  }
...
void LinkZone(int x, int y, int linkto, int dir)
{
  long int WildRoom = real_room(MAPZONE + (y * MAPY) + x);
  long int ZoneRoom = real_room(linkto);
  long int WildRoom2;
  int revdir = -1;
...
//    x--;
  ...
  WildRoom2 = real_room(MAPZONE + (y * MAPY) + x);

  // Set flag in zone room
  world[ZoneRoom].sector_type = SECT_INSIDE;
  world[ZoneRoom].wild_type = WILD_ZONEENT;
  world[WildRoom].sector_type = SECT_INSIDE;
  world[WildRoom].wild_type = WILD_ZONEENT;


  if (world[ZoneRoom].dir_option[dir])
    free(world[ZoneRoom].dir_option[dir]);

  CREATE(world[ZoneRoom].dir_option[dir], struct room_direction_data, 1);
  world[ZoneRoom].dir_option[dir]->to_room = WildRoom2;
//  world[ZoneRoom].dir_option[dir]->exit_info = world[WildRoom2].dir_option[revdir]->exit_info;


  if (world[WildRoom2].dir_option[revdir])
    free(world[WildRoom2].dir_option[revdir]);

  CREATE(world[WildRoom2].dir_option[revdir], struct room_direction_data, 1);
  world[WildRoom2].dir_option[revdir]->to_room = ZoneRoom;
//  world[WildRoom2].dir_option[revdir]->exit_info = world[ZoneRoom].dir_option[dir]->exit_info;
//
 log("zones linked");
}
 ...
 /* Starting Points top left coordinates of displayed map */
  ...
  for (y = StartY + YBufLen, y1 = 0 ; y1 <= YBufLen ; y--, y1++) {
    ...
    /* Take care of the corners of the map */
        ...
        send_to_char(ch,"%s",	flags[world[real_room(MAPZONE + (y * MAPY) + x)].wild_type]);
      }
      if (x1 == XBufLen)
        send_to_char(ch,"\r\n");
    }
...
void LoadMapImage(void)
{
  ...
  log("map image loaded top world = %ld top wild = %d, chars count = %ld", top_of_world, Top_of_Wild, charcount);
  fclose(MapImage);
  gdImageDestroy(im);
  return;
}
...
void PopulateWild(void)
{
 ...
 for(i=Top_of_Wild;i > (top_of_world - Top_of_Wild);i--)
 {
   ...
   {  
    mobnum = real_mobile(wildmobs[mob]);
