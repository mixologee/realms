  ...
  if (IS_NPC(ch)) {
    ...
    gain = graf(age(ch)->year, 4, 8, 12, 16, 12, 10, 8);
     ...
     for (i=0; i < NUM_WEARS; i++) {
       if (GET_EQ(ch, i)) {
         if(IS_SET_AR(GET_OBJ_EXTRA(GET_EQ(ch,i)), ITEM_MANAREGEN)) { 
//         if (GET_OBJ_EXTRA(GET_EQ(ch, i)) == ITEM_MANAREGEN) {
           gain += rand_number(5, 15); /* baseline for testing */
         }
       }
     }
...
int hit_gain(struct char_data *ch)
{
  ...
  if (IS_NPC(ch)) {
    ...
    for (i=0; i < NUM_WEARS; i++) {
       if (GET_EQ(ch, i)) {
         if(IS_SET_AR(GET_OBJ_EXTRA(GET_EQ(ch, i)), ITEM_HITPREGEN)) { 
//         if (GET_OBJ_EXTRA(GET_EQ(ch, i)) == ITEM_HITPREGEN) {
           gain += rand_number(5,15); /*baseline for testing */
         }
       }
    }
...
int move_gain(struct char_data *ch)
{
  ...
  if (AFF_FLAGGED(ch, AFF_POISON))
    ...
    for (i=0; i < NUM_WEARS; i++) {
       if (GET_EQ(ch, i)) {
         if(IS_SET_AR(GET_OBJ_EXTRA(GET_EQ(ch, i)), ITEM_HITPREGEN) || IS_SET_AR(GET_OBJ_EXTRA(GET_EQ(ch, i)), ITEM_MANAREGEN)) {
//         if (GET_OBJ_EXTRA(GET_EQ(ch, i)) == ITEM_HITPREGEN) {
           gain += rand_number(5,15); /*baseline for testing */
         }
       }
...
{
  if (GET_EMAIL(ch) != NULL)
    free(GET_EMAIL(ch));

  if (mail_addr == NULL || !(*mail_addr)) {
    GET_EMAIL(ch) = NULL;
  } else {
    if (strlen(mail_addr) > MAX_EMAIL_LENGTH)
      mail_addr[MAX_EMAIL_LENGTH] = '\0';

    GET_EMAIL(ch) = strdup(mail_addr);
  }
}
...
void run_autowiz(void)
{
  ...
  if (CONFIG_USE_AUTOWIZ) {
    ...
    if (res < sizeof(buf)) {
      nmudlog(SYS_LOG, LVL_GOD, TRUE, "Initiating Autowiz");
...
void gain_exp(struct char_data *ch, int gain)
{
  ...
  if (IS_NPC(ch)) {
    GET_EXP(ch) += gain * exp_multiplier;
    return;
  }
  if (gain > 0) {
    ...
    if (is_altered) {
      nmudlog(INFO_LOG, LVL_GOD, TRUE, "%s advanced %d level%s to level %d", GET_NAME(ch), num_levels, num_levels == 1 ? "" : "s", GET_LEVEL(ch));
...
void check_idling(struct char_data *ch)
{
  if (++(ch->char_specials.timer) > CONFIG_IDLE_VOID) {
	...
	Crash_idlesave(ch);
      nmudlog(INFO_LOG, LVL_GOD, TRUE, "%s force-rented and extracted (idle)", GET_NAME(ch));
