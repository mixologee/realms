...
ROOM_DATA *create_wild_room( COORD_DATA *coord, bool Static )
{
	...
	{
		if (Static && !ROOM_FLAGGED(wRoom->number, ROOM_WILD_STATIC))
		{
			SET_BIT_AR(ROOM_FLAGS(wRoom->number), ROOM_WILD_STATIC);
			SET_BIT_AR(ROOM_FLAGS(wRoom->number), ROOM_NOMAGIC);
		}
...
void wild_remove_enqueue( ROOM_DATA *wRoom )
{
	...
	SET_BIT_AR( ROOM_FLAGS( wRoom->number ), ROOM_WILD_REMOVE );
}
...
void wild_remove_dequeue( ROOM_DATA *wRoom )
{
	...
	if ( !wRoom->coord )
		return;
	if ( !ROOM_FLAGGED( wRoom->number, ROOM_WILD_REMOVE ) )
		return;
	...
	for ( wrem = wild_remove_list; wrem; wrem = wrem_next )
	{
		...
		if ( wrem->wRoom == wRoom )
		{
			REMOVE_BIT_AR( ROOM_FLAGS(wRoom->number), ROOM_WILD_REMOVE );
			REMOVE_FROM_LIST( wrem, wild_remove_list, next );
			DISPOSE( wrem );
		}
	}
...
void wild_check_for_remove( ROOM_DATA *wRoom )
{
	if ( !wRoom || !IS_WILD( wRoom ) )
		return;
	if ( ROOM_FLAGGED( wRoom->number, ROOM_WILD_STATIC ) ||
	     ROOM_FLAGGED( wRoom->number, ROOM_WILD_REMOVE ) )
		return;
	if ( !wRoom->coord )
		return;
...
void setup_wild_static( ROOM_DATA *wRoom, ROOM_DATA *zRoom )
{
	...
	for ( around.y = wRoom->coord->y - 1; around.y < wRoom->coord->y + 2; around.y++ )
	{
		for ( around.x = wRoom->coord->x - 1; around.x < wRoom->coord->x + 2; around.x++ )
		{
			...
			{
				SET_BIT_AR( ROOM_FLAGS( nRoom->number ), ROOM_WILD_STATIC );
				SET_BIT_AR( ROOM_FLAGS( nRoom->number ), ROOM_NOMAGIC );
			}
		}
	}
					 ...
					 ROOM_DATA *pRoomFrom, ROOM_DATA *pRoomTo)
{
	...
	quady[0] = GET_RY(pRoomFrom->number) / MAP_SIZE * MAP_SIZE;
	quadx[0] = GET_RX(pRoomFrom->number) / MAP_SIZE * MAP_SIZE;
	
	quady[1] = GET_RY(pRoomTo->number) / MAP_SIZE * MAP_SIZE;
	quadx[1] = GET_RX(pRoomTo->number) / MAP_SIZE * MAP_SIZE;
