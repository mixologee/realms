...
void auth_aboard( SHIP_DATA *ship, CHAR_DATA *ch, CHAR_DATA *vict, int mode )
{
	...
	else
	{
		...
		{
			send_to_char(ch, "You cannot revoke authorization to someone that's aboard.\r\n");
			return;
		}
...
void ship_to_room( SHIP_DATA *ship, ROOM_DATA *room )
{
	...
	if ( IS_WILD(room) )
	{
		...
		if (ROOM_FLAGGED(room->number, ROOM_WILD_REMOVE))
...
ACMD(do_stell)
{
	...
	if ( !ch->in_ship )
	{
		send_to_char(ch, "You aren't embarked on a ship.\r\n");
		return;
	}

	if (!*arg)
	{
		send_to_char(ch, "What do you want to tell to people on ship?\r\n");
		return;
	}
...
ACMD(do_embark)
{
	...
	if ( !*arg )
	{
		send_to_char(ch, "Embark on which ship?\r\n");
		return;
	}

	if ( ch->in_ship )
	{
		send_to_char(ch, "But you are currently embarked on a ship.. disembark first.\r\n");
		return;
	}

	if ( !world[IN_ROOM(ch)].ships )
	{
		send_to_char(ch, "No ships here.\r\n" );
		return;
	}
	...
	if ( RIDING(ch) )
	{
		send_to_char(ch, "No way you can go aboard while riding.\r\n");
		return;
	}

	if ( WAGONER(ch) )
	{
		send_to_char(ch, "No way you can go aboard while driving a vehicle.\r\n");
		return;
	}
...
ACMD(do_disembark)
{
	...
	if ( !ch->in_ship )
	{
		send_to_char(ch,  "But you aren't aboard a ship.\r\n");
		return;
	}

	if ( !SHIP_FLAGGED(ch->in_ship, SHIP_IN_PORT) )
	{
		send_to_char(ch,  "You cannot disembark from a ship that's not in a port.\r\n");
		return;
	}

	if ( SECT(ch->in_room) != SECT_SHIP_STERN )
	{
		send_to_char(ch,  "You must be astern to disembark.\r\n");
		return;
	}
	...
	if ( !( pRoom = get_wild_room( ch->in_ship->port ) ) )
	{
		...
		send_to_char(ch,  "Sorry, at the moment you cannot disembark.\r\n");
		return;
	}
...
ACMD(do_courselist)
{
	...
	if ( !course_table )
	{
		send_to_char(ch, "No course loaded.\r\n");
		return;
	}
...
ACMD(do_ship)
{
	...
	if (!ch->in_ship)
	{
		send_to_char(ch, "To be aboard a ship would surely help.. he he\r\n");
		return;
	}
	...
	if (is_abbrev(arg, "drive"))
	{
		if ( SECT(ch->in_room) != SECT_SHIP_STERN )
		{
			send_to_char(ch, "You must be at the stern to drive a ship.\r\n");
			return;
		}

		if ( GET_IDNUM(ch) != ship->idcaptain )
		{
			send_to_char(ch, "You must be the captain to drive the ship.\r\n");
			return;
		}

		if ( !GET_SKILL(ch, SKILL_NAVIGATION) )
		{
			send_to_char(ch, "You have no idea of how to drive a ship.\r\n");
			return;
		}

		if ( GET_SKILL(ch, SKILL_NAVIGATION) < ship->type->skill_min )
		{
			send_to_char(ch,"You aren't skilled enough to drive a ship of this size.\r\n");
			return;
		}
		...
		{
			send_to_char(ch, "Someone else is driving the ship.\r\n");
			return;
		}
		...
		if ( ch->player_specials->athelm )
		{
			...
			send_to_char(ch, "You let off the helm.\r\n");
		}
		else
		{
			ch->player_specials->athelm	= TRUE;
			ship->helmperson			= ch;
			send_to_char(ch, "You grab the helm and start to drive the ship.\r\n");
		}
	}
	else if (is_abbrev(arg, "status"))
	{
		...
		sprintbit(ship->flags, ship_flags, sflags, sizeof(sflags));
		sprintf(buf,
			"---------------------------------------------------------------\r\n"
			"Name   : '%s'\r\nType   : %s\r\nFlags  : %s\r\n",
	...
	else if (is_abbrev(arg, "sail"))
	{
		...
		{
			send_to_char(ch, "Anchored ships cannot move.\r\n");
			return;
		}

		if ( SHIP_FLAGGED(ship, SHIP_IN_COURSE) )
		{
			send_to_char(ch, "You are already following a course, use <ship stop> to stop the ship.\r\n");
			return;
		}
		...
		if ( !GET_SKILL(ch, SKILL_NAVIGATION) )
		{
			send_to_char(ch, "You have no idea of how set a course.\r\n");
			return;
		}
		...
		if ( !*arg2 )
		{
			...
			send_to_char(ch, "Sail to which port?\r\n");
			return;
		}
		...
		if ( SHIP_FLAGGED(ship, SHIP_IN_PORT) )
		{
			...
			{
				send_to_char(ch, "The shorter history sail. There you are.\r\n");
				return;
			}
		}
		...
		{
			send_to_char(ch, "You can follow sea courses only when leaving a port.\r\n");
			return;
		}
	...
	else if (!str_cmp(arg, "stop"))
	{
		if ( !SHIP_FLAGGED(ship, SHIP_IN_COURSE))
		{
			send_to_char(ch,"You aren't following a course.\r\n");
			return;
		}
		...
		send_to_char(ch, "You stop follow the sea course.\r\n");
	...
	else if (is_abbrev(arg, "captain"))
	{
		...
		if ( !*arg2 )
		{
			...
			{
				ship->idcaptain = -1;
				send_to_char(ch, "The ship is now without a captain.\r\n");
			}
			else
				send_to_char(ch, "Nominate captain who?\r\n");
			return;
		}

		if ( ship->idcaptain != -1 )
		{
			send_to_char(ch, "The ship already has a captain.\r\n");
			return;
		}

		if ( !SHIP_FLAGGED(ship, SHIP_IN_PORT) || !( pRoom = get_wild_room(ship->port) ) )
		{
			send_to_char(ch, "You can nominate only when ship is at one port.\r\n");
			return;
		}
		...
		{
			send_to_char(ch,"You cannot nominate captain someone you don't know.\r\n");
			return;
		}
	...
	else if (is_abbrev(arg, "anchor"))
	{
		...
		{
			send_to_char(ch,"You must be the captain to place/remove the anchor.\r\n");
			return;
		}
		...
		if (SHIP_FLAGGED(ship, SHIP_AT_ANCHOR))
		{
			...
			send_to_char(ch,"You remove the anchor.\r\n");
		}
		else
		{
			if ( SHIP_FLAGGED(ship, SHIP_IN_COURSE))
			...
			send_to_char(ch,"You place the anchor.\r\n");
		}
	}
	else
		send_to_char(ch, "Not yet implemented, sorry.\r\n");
}
...
bool ship_can_sail( CHAR_DATA *ch, SHIP_DATA *ship )
{
	...
	{
		send_to_char(ch, "The ship cannot sail, too weight aboard.\r\n");
		return (FALSE);
	}
	...
	{
		send_to_char(ch, "The ship is too damaged to leave the port.\r\n");
		return (FALSE);
	}
...
void ship_to_port( SHIP_DATA *ship, ROOM_DATA *pRoom )
{
	...
	{
		log( "SYSERR: no port found at coord %d %d.", GET_RY(pRoom->number), GET_RX(pRoom->number) );
	...
	ship->port->y = GET_RY(pRoom->number);
	ship->port->x = GET_RX(pRoom->number);
	
	sprintf( buf, "%s sails to %s.\r\n", ship->name, port->name );
	send_to_room(ship->in_room, buf);
...
void engage_course( CHAR_DATA *ch, COURSE_DATA *course, bool recover )
{
	...
	if ( ship->action )
	{
		send_to_char(ch, "You can't sail at the moment.\r\n" );
		return;
	}
	...
	{
		send_to_char(ch,"You can't go on landmasses with a ship.\r\n");
		ship->course	= NULL;
		ship->cstep		= NULL;
		return;
	}
	...
	if ( SECT(ship->in_room->number) == SECT_PORT )
	{
...
void ship_move( CHAR_DATA *ch, int dir, int special_check )
{
	...
	if ( ship->action )
	{
		if ( SHIP_FLAGGED(ship, SHIP_IN_COURSE ) )
		{
			...
			ship->action = NULL;
			REMOVE_BIT(SHIP_FLAGS(ship), SHIP_IN_COURSE);
			send_to_char(ch, "You stop the course sailing.\r\n");
		}
		else
		{
			send_to_char(ch,"The ship is currently performing another action.\r\n");
			return;
		}
	}
	...
	/* first move out of the port */
	if ( world[ship->in_room->number].sector_type == SECT_PORT )
	{
		REMOVE_BIT(SHIP_FLAGS(ship), SHIP_IN_PORT);
		SET_BIT(SHIP_FLAGS(ship), SHIP_SAIL);
		DISPOSE(ship->port);
...
bool create_ship_rooms(SHIP_DATA *ship)
{
	...
	for ( ;; )
	{
		...
		ship->rooms[room]->description	= fread_string_nospace(fp);
                ...
                ship->rooms[room]->room_flags[0] = asciiflag_conv(flags);
                ship->rooms[room]->room_flags[1] = asciiflag_conv(flags2);
                ship->rooms[room]->room_flags[2] = asciiflag_conv(flags3);
                ship->rooms[room]->room_flags[3] = asciiflag_conv(flags4);
...
ACMD(do_shipsetup)
{
	...
	if (shipname == NULL)
	{
		send_to_char(ch,"Ship names must be enclosed in the Holy Magic Symbols: '\r\n");
		return;
	}
	...
	if ( !pRoom || world[pRoom->number].sector_type != SECT_PORT )
	{
		send_to_char(ch,"Invalid coordinates.\r\n");
		return;
	}
...
void fwrite_ship( SHIP_DATA *ship )
{
	...
	if ( SHIP_FLAGGED(ship, SHIP_IN_PORT) )
		...
		fprintf( fp, "Anchored       %hd %hd\n", GET_RY(ship->in_room->number), GET_RX(ship->in_room->number) );
	else
...
SPECIAL(dock)
{
	if (CMD_IS("list"))
	{
		...
		send_to_char(ch,"You can buy the following ships:\r\n");
		for ( y = 0; y < top_ship_table; y++ )
			ch_printf(ch, "  %-20s %d\r\n", ship_table[y]->name,
	...
	if (CMD_IS("buy"))
	{
		...
		if ( !*arg || !*shipname )
		{
			send_to_char(ch,"Usage: buy <type> 'ship name'\r\n");
			return (1);
		}

		if (!(port = get_port_by_coord(world[IN_ROOM(ch)].coord)))
		{
...
//		if ( coord->y == GET_RY(port) && coord->x == GET_RX(port) )
		if ( coord->y == port->coord->y && coord->x == port->coord->x )
			break;
...
void load_ports( void )
{
	...
	for ( port = port_table; port; port = port->next )
	{
		...
		pRoom->func = dock;
		if ( SECT(pRoom->number) != SECT_PORT )
		{
			put_sect( GET_RY(pRoom->number), GET_RX(pRoom->number), SECT_PORT, TRUE );
			world[pRoom->number].sector_type = SECT_PORT;
		}
	}
...
EVENTFUNC(ferry_move_event)
{
	...
	send_to_room(pFerry->in_room, "The ferryboat has left."); 
	...
	if ( IS_WILD(pFerry->in_room) )
	{
		...
		if (ROOM_FLAGGED(pFerry->in_room->number, ROOM_WILD_REMOVE))
			wild_remove_dequeue(pFerry->in_room);
		if (!ROOM_FLAGGED(pFerry->in_room->number, ROOM_WILD_STATIC))
			SET_BIT_AR(ROOM_FLAGS(pFerry->in_room->number), ROOM_WILD_STATIC);
	}
...
void ferry_move(void)
{
	...
	for (pFerry = first_ferry; pFerry; pFerry = pFerry->next)
	{
		...
		send_to_room( pFerry->room->number, "The ferryboat leave the port and begin to cross the river.\r\n");
		send_to_room( pFerry->in_room->number, "The ferryboat leave the port and begin to cross the river.\r\n");
	}
}
...
void LoadFerryboats( void )
{
	...
	while (!feof(fp))
	{
		...
		else
		{
			...
			if (pFerry->mob)
				char_to_room(pFerry->mob, pFerry->room->number);
