...
ACMD(do_oasis_clanedit)
{
  ...
  if(number == -1) {
    ...
    for (cptr = clan_info; cptr && cptr->number != number; cptr=cptr->next);
      if (cptr && (cptr->number == number)) {
        ...
        send_to_char(d->character, "Invalid clan number! cptr = %d, arg = %s, buf1 = %s\r\n", cptr->number, argument, buf1);
        return;
      }
  }
  ...
  nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s starts editing clans", GET_NAME(ch));
//  mudlog(BRF, LVL_IMMORT, TRUE, "OLC: %s starts editing clans.", GET_NAME(ch));
 
  send_to_char(ch, "Saving clans.\r\n");
  nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s saves clans", GET_NAME(ch));
//  mudlog(CMP, MAX(LVL_BUILDER, GET_INVIS_LEV(ch)), TRUE,  "OLC: %s saves the clans.", GET_NAME(ch));
...
void clanedit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case CLANEDIT_CONFIRM_SAVE:
    ...
    case 'Y':
      save_clans();
      nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s edits clan %d", GET_NAME(d->character), OLC_CLAN(d)->number);
     ...
     case CLANEDIT_RANK1:
      ...
      case CLANEDIT_ENTR_ROOM:
        ...
        if (number <= 0 || number > NUM_STARTROOMS+1) 
          ...
          nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s edits the HOMETOWN for clan %d", GET_NAME(d->character), OLC_CLAN(d)->number);
//          mudlog(CMP, LVL_GOD, TRUE, "OLC: %s edits HOMETOWN for clan %d", GET_NAME(d->character), OLC_CLAN(d)->number);
     ...
     case CLANEDIT_RECALL:
      ...
      default:
        ...
        nmudlog(OLC_LOG, LVL_GOD, TRUE, "SYSERR: Reached default case in clanedit_parse");
