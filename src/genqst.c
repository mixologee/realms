...
int copy_quest(struct aq_data *to, struct aq_data *from, int free_old_strings)
{
  ...
  to->value[3] = from->value[3];
  to->value[4] = from->value[4];
  to->value[5] = from->value[5];
  to->value[6] = from->value[6];
//  for (i = 0; i < 7; i){
...
int delete_quest(qst_rnum rnum)
{
  ...
  if (rnum >= total_quests)
    return FALSE;
  log("GenOLC: delete_quest: Deleting quest #%ld (%s).",
...
int save_quests(zone_rnum zone_num)
{
...
#endif
    log("SYSERR: GenOLC: save_quests: Invalid zone number %ld passed! (0-%ld)",
         zone_num, top_of_zone_table);
    return FALSE;
  }

  log("GenOLC: save_quests: Saving quests in zone #%ld (%ld-%ld).",
        zone_table[zone_num].number,
	genolc_zone_bottom(zone_num), zone_table[zone_num].top);

  snprintf(filename, sizeof(filename), "%s/%ld.new",
  ...
  snprintf(oldname, sizeof(oldname), "%s/%ld.qst",
           QST_PREFIX, zone_table[zone_num].number);
  remove(oldname);
