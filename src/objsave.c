...
struct obj_data *read_plr_object(obj_vnum nr, int type);
...
int objsave_save_obj_record(struct obj_data *obj, FILE *fp, int locate)
{
  ...
  if (GET_OBJ_VNUM(obj) != NOTHING)
    temp=read_plr_object(GET_OBJ_VNUM(obj), VIRTUAL);
  else {
    temp = create_obj();
  ...
  if (location > 0) {  /* Was wearing it. */
    ...
    case WEAR_HOLD:
      if (CAN_WEAR(obj, ITEM_WEAR_HOLD))
        break;
      if (IS_WARRIOR(ch) && CAN_WEAR(obj, ITEM_WEAR_WIELD) && GET_OBJ_TYPE(obj) == (ITEM_WEAPON || ITEM_CROSSBOW || ITEM_BOW || ITEM_SLING))
        break;
      location = LOC_INVENTORY;
      break;
    case WEAR_AURA:
...
void Crash_listrent(struct char_data *ch, char *name)
{
  ...
  default:
	...
	for (current = loaded; current != NULL; current=current->next)
	  len += snprintf(buf+len, sizeof(buf)-len, "[%5d] (%5ddamage) %-20s\r\n",
                GET_OBJ_VNUM(current->obj),
                GET_OBJ_COND(current->obj),
...
void Crash_extract_expensive(struct obj_data *obj)
{
  ...
  for (tobj = obj; tobj; tobj = tobj->next_content)
    if (GET_OBJ_COND(tobj) > GET_OBJ_COND(max))
      max = tobj;
//  extract_obj(max);
}
...
int objsave_write_rentcode(FILE *fl, int rentcode, int condition, struct char_data *ch)
{
      ...
      *cost = 0;
      if (display) {
        sprintf(buf, "$n tells you, '%5d coins for %s..'",
                (GET_OBJ_COND(obj) * factor), OBJS(obj, ch));
        act(buf, FALSE, recep, 0, ch, TO_VICT);
      }
    }
...
obj_save_data *objsave_parse_objects(FILE *fl)
{
  ...
  while (TRUE) {
    ...
    if (*line == '#') {
      ...
      if (nr == NOTHING) {   /* then it is unique */
        ...
        if(real_object(nr) != NOTHING) {
          temp=read_plr_object(nr,VIRTUAL);
    ...
    case 'C':
      ...
      else if (!strcmp(tag, "Cond"))
        GET_OBJ_COND(temp) = num;
      break;
    case 'D':
      if (!strcmp(tag, "Desc"))
