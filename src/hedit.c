...
ACMD(do_oasis_hedit)
{
  ...
  if (!str_cmp("save", arg)) {
    nmudlog(OLC_LOG, LVL_GOD, TRUE, "HELP SYSTEM: %s saved the help files.", GET_NAME(ch));
...
//           GET_NAME(ch));
  ...
  if (d->olc) {
    nmudlog(OLC_LOG, LVL_GOD, TRUE, "SYSERR: do_oasis: Player already had OLC structure.");
//    mudlog(BRF, LVL_IMMORT, TRUE, "SYSERR: do_oasis: Player already had olc structure.");
  ...
  nmudlog(OLC_LOG, LVL_GOD, TRUE, "HELPSYS: %s starts editing help files", GET_NAME(d->character));
...
void hedit_save_internally(struct descriptor_data *d)
{
  ...
  if (OLC_ZNUM(d) == NOWHERE /*> top_of_helpt*/) {
    int i;
...
void hedit_save_to_disk(struct descriptor_data *d)
{
  ...
  if (in_save_list(HEDIT_PERMISSION, SL_HLP))
  ...
  case HEDIT_CONFIRM_SAVESTRING:
    ...
    case 'Y':
      snprintf(buf, sizeof(buf), "OLC: %s edits help for %s.", GET_NAME(d->character),
               OLC_HELP(d)->keywords);
      nmudlog(OLC_LOG, LVL_GOD, TRUE, "HELPSYS: %s edits the help file for %s", GET_NAME(d->character), OLC_HELP(d)->keywords);
//      mudlog(TRUE, MAX(LVL_BUILDER, GET_INVIS_LEV(d->character)), CMP, buf);
  ...
  case HEDIT_ENTRY:
    ...
    nmudlog(OLC_LOG, LVL_GOD, TRUE, "HELPSYS: ERROR: Reached HEDIT_ENTRY case in parse_hedit");
//    mudlog(TRUE, LVL_BUILDER, BRF, "SYSERR: Reached HEDIT_ENTRY case in parse_hedit");
  ...
  default:
    ...
    nmudlog(OLC_LOG, LVL_GOD, TRUE, "HELPSYS: ERROR: Reached default case in parse_hedit");
