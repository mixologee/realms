...
void LoadStableRentIndex(void)
{
	...
	if (!(fp = fopen(iname, "r")))
	{
		log("ERROR:  No Stables actually in game. stables.c in LoadStableRentIndex");
		return;
	}
...
void rent_mount(CHAR_DATA *ch, CHAR_DATA *mount)
{
	...
	strent->stable_vnum	= world[IN_ROOM(ch)].number;
	...
	for ( ; ; )
	{
		...
		if ( !strcmp( word, "VEHICLE" ) )
		{
			if ( !( vehicle = fread_rent_vehicle(fp, ch->in_room) ) )
			{
				...
				send_to_char(ch, "There is a problem with your vehcile. Tell an Immortal.\r\n");
				fclose( fp );
				return;
			}
		...
		else if ( !strcmp( word, "MOUNT" ) )
		{
			if ( !( mount = fread_rent_mount(fp, ch->in_room) ) )
			{
				...
				send_to_char(ch, "There is a problem with your mount. Tell an Immortal.\r\n");
				fclose( fp );
				return;
			}
		}
...
ACMD(do_stablerent)
{
	...
	if ( !first_stable_rent )
	{
		send_to_char(ch, "No mounts rented in stables at the moment.\r\n");
		return;
	}
