...
ACMD(do_oasis_zedit)
{
  ...
  if (d->olc) {
    nmudlog(OLC_LOG, LVL_GOD, TRUE, "SYSERR: do_oasis_zedit: Player already had olc structure.");
    free(d->olc);
  }
  ...
  if (save) {
    ...
    nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s saved zone info for zone %d", GET_NAME(ch), zone_table[OLC_ZNUM(d)].number);
  ...
  if (GET_OLC_ZONE(ch) == -1)
    nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s starts editing zone %d - Allowance: ALL", GET_NAME(ch), zone_table[OLC_ZNUM(d)].number);
 else
  nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s starts editing zone %d - Allowance: %d", GET_NAME(ch),  zone_table[OLC_ZNUM(d)].number, GET_OLC_ZONE(ch)); 
}
...
void zedit_new_zone(struct char_data *ch, zone_vnum vzone_num, room_vnum bottom, room_vnum top)
{
  ...
  nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s creates new zone #%d", GET_NAME(ch), vzone_num);
...
//	  "%sR%s) Reset Mode     : %s%s%s\r\n"
	  ...
	  cyn, zone_table[OLC_ZNUM(d)].number,
          zone_table[OLC_ZNUM(d)].x, zone_table[OLC_ZNUM(d)].y,
	  grn, nrm, yel, OLC_ZONE(d)->builders ? OLC_ZONE(d)->builders : "None.",
//        grn, nrm, yel, OLC_ZONE(d)->type ? OLC_ZONE(d)->builders " "Unset",
//        grn, nrm, yel, OLC_ZONE(d)->lowlev,
//        grn, nrm, yel, OLC_ZONE(d)->highlev,
  ...
  while (MYCMD.command != 'S') {
    ...
    case 'D':
      write_to_output(d, "%sSet door %s as %s.",
              MYCMD.if_flag ? " then " : "",
              dirs[MYCMD.arg2], door_stat[MYCMD.arg3]);
      break;
    case 'T':
...
void zedit_disp_arg1(struct descriptor_data *d)
{
  ...
  default:
    ...
    nmudlog(OLC_LOG, LVL_GOD, TRUE, "SYSERR: zedit_disp_arg1()");
...
void zedit_disp_arg2(struct descriptor_data *d)
{
  ...
  default:
    ...
    cleanup_olc(d, CLEANUP_ALL);
    nmudlog(OLC_LOG, LVL_GOD, TRUE, "SYSERR: zedit_disp_arg2()");
    write_to_output(d, "Oops...\r\n");
    return;
  }
...
void zedit_disp_arg3(struct descriptor_data *d)
{
  ...
  default:
    ...
    cleanup_olc(d, CLEANUP_ALL);
    nmudlog(OLC_LOG, LVL_GOD, TRUE, "SYSERR: zedit_disp_arg3()");
    write_to_output(d, "Oops...\r\n");
    return;
  }
...
/* Print out the levels menu and setup the response catch */
...
void zedit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case ZEDIT_CONFIRM_SAVESTRING:
    ...
    case 'Y':
      ...
      nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s edits zone info for room %d", GET_NAME(d->character), OLC_NUM(d));
      /* FALL THROUGH */
    case 'n':
    case 'N':
      cleanup_olc(d, CLEANUP_ALL);
...
#define Z_MX_LVL 100
  case ZEDIT_LEVELS:
    switch (*arg) {
    case '1': write_to_output(d, "Enter the min level for this zone (0-%d, -1 = none): ", Z_MX_LVL);
              OLC_MODE(d) = ZEDIT_LEV_MIN;
              break;

    case '2': write_to_output(d, "Enter the max level for this zone (0-%d, -1 = none): ", Z_MX_LVL);
...
/*-------------------------------------------------------------------*/
  ...
  case ZEDIT_ARG1:
    ...
    case 'E':
    case 'G':
      if ((pos = real_object(((long int) atoi(arg)))) != NOTHING) {
	OLC_CMD(d).arg1 = pos;
	zedit_disp_arg2(d);
      } else
    ...
    default:
      /* We should never get here. */
      cleanup_olc(d, CLEANUP_ALL);
      nmudlog(OLC_LOG, LVL_GOD, TRUE, "SYSERR: zedit_parse(): case ARG1");
      write_to_output(d, "Oops...\r\n");
      break;
    }
    break;
  ...
  case ZEDIT_ARG2:
    ...
    default:
      /* We should never get here, but just in case. */
      cleanup_olc(d, CLEANUP_ALL);
      nmudlog(OLC_LOG, LVL_GOD, TRUE, "SYSERR: zedit_parse(): case ARG2");
      write_to_output(d, "Oops...\r\n");
      break;
    }
    break;
  ...
  case ZEDIT_ARG3:
    ...
    default:
      /* We should never get here, but just in case. */
      cleanup_olc(d, CLEANUP_ALL);
      nmudlog(OLC_LOG, LVL_GOD, TRUE, "SYSERR: zedit_parse(): case ARG3");
      write_to_output(d, "Oops...\r\n");
      break;
    }
    break;
  ...
  case ZEDIT_Y_COORD:
    ...
    if (genolc_checkstring(d, arg)) {
      if (OLC_ZONE(d)->name)
        ...
        log("SYSERR: OLC: X COORD: no coord to free!");
      OLC_ZONE(d)->y = atoi(arg);//((int) strdup(arg));
      OLC_ZONE(d)->number = 1;
    }
    zedit_disp_menu(d);
    break;
  ...
  case ZEDIT_X_COORD:
    ...
    if (genolc_checkstring(d, arg)) {
      if (OLC_ZONE(d)->name)
        ...
        log("SYSERR: OLC: Y COORD: no coord to free!");
      OLC_ZONE(d)->x = atoi(arg); //(int) strdup(arg));
      OLC_ZONE(d)->number = 1;
    }
    zedit_disp_menu(d);
    break;
  ...
  case ZEDIT_ZONE_TOP:
    ...
    if (OLC_ZNUM(d) == top_of_zone_table)
      OLC_ZONE(d)->top = LIMIT((long int) atoi(arg), genolc_zonep_bottom(OLC_ZONE(d)), 9999999);
    else
  ...
  default:
    /* We should never get here, but just in case... */
    cleanup_olc(d, CLEANUP_ALL);
    nmudlog(OLC_LOG, LVL_GOD, TRUE, "SYSERR: zedi_parse(): Reached Default Case!!");
    write_to_output(d, "Oops...\r\n");
    break;
  }
}
...
bool zedit_get_levels(struct descriptor_data *d, char *buf)
{
  ...
  if ((OLC_ZONE(d)->min_level == -1) && (OLC_ZONE(d)->max_level == -1)) {
    sprintf(buf, "&R<Not Set!>&n");
    return FALSE;
  }

  if (OLC_ZONE(d)->min_level == -1) {
    sprintf(buf, "Up to level &M%d&n", OLC_ZONE(d)->max_level);
    return TRUE;
  }

  if (OLC_ZONE(d)->max_level == -1) {
    sprintf(buf, "Above level &M%d&n", OLC_ZONE(d)->min_level);
    return TRUE;
  }
