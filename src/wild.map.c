...
int char_to_sect(char charsect)
{
	...
	case 'M': nsect = SECT_SNOW_MOUNTAIN;		break;
	case 'T': nsect = SECT_ROCK_MOUNTAIN;	break;
	case 'A': nsect = SECT_HIGHLANDS;				break;
	case 'p': nsect = SECT_PLAINS;				break;
	...
	case '-': nsect = SECT_SHALLOWS;			break;
        case '$': nsect = SECT_TRADEPOST;                       break;
...
char sect_to_char(int sectnum)
{
	...
	case SECT_SNOW_MOUNTAIN:		nsect = 'M';	break;
	case SECT_ROCK_MOUNTAIN:	nsect = 'T';	break;
	case SECT_HIGHLANDS:				nsect = 'A';	break;
	case SECT_PLAINS:				nsect = 'p';	break;
	...
	case SECT_DESERT:				nsect = 'D';	break;
        case SECT_TRADEPOST:                            nsect = '$';    break;
...
char *show_sect(CHAR_DATA *ch, char sect, COORD_DATA *coord, bool plain, int iDist)
{
	...
	if ( (pRoom->buildings || ROOM_FLAGGED(pRoom->number, ROOM_BUILDING_WORKS)) && pRoom->sector_type != SECT_CITY)
	{
		strcpy(simbolo, "&M");
	}
	...
	{
		if (!get_room_aff_bitv(pRoom, RAFF_SHELTER))
			strcpy(simbolo, "&Y");
	}
...
void show_ship_on_map( CHAR_DATA *ch )
{
	...
	y		= GET_RY(ch->in_ship->in_room->number);
	x		= GET_RX(ch->in_ship->in_room->number);
...
void show_map(CHAR_DATA *ch)
{
	...
	if (IN_FERRY(ch))
		 pFerry = get_ferry(world[IN_ROOM(ch)].extra_data->vnum);

	x = y = xmax = ymax = xmin = ymin = 0;

	if ( ON_DECK(ch) )
	{
		chy		= GET_RY(ch->in_ship->in_room->number);
		chx		= GET_RX(ch->in_ship->in_room->number);
	}
	else if ( IN_FERRY(ch) )
	{
		chy		= GET_RY(pFerry->in_room->number);
		chx		= GET_RX(pFerry->in_room->number);
	}
	else
	{
		chy		= GET_Y(ch);
	...
	else
	{
		...
		if (ON_DECK(ch))
			...
			sky = get_coord_weather(world[IN_ROOM(ch)].coord);
...
void look_at_wild(CHAR_DATA *ch, bool quick)
{
	...
	if (world[IN_ROOM(ch)].portal_stone)
		ch_printf(ch, "%s\r\n", world[IN_ROOM(ch)].portal_stone->description);
	if (world[IN_ROOM(ch)].ferryboat)
		send_to_char(ch, "&b&4You see a ferryboat.&0\r\n");

	/* now list characters & objects */
	list_building_to_char(world[IN_ROOM(ch)].buildings, ch);
	list_ship_to_char(world[IN_ROOM(ch)].ships, ch);
	list_vehicle_to_char(world[IN_ROOM(ch)].vehicles, ch);
	list_obj_to_char(world[IN_ROOM(ch)].first_content, ch, 0, FALSE);
