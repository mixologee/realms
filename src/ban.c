...
ACMD(do_ban)
{
  ...
  nmudlog(GOD_LOG, LVL_GOD, TRUE, "%s banned %s for %s players", GET_NAME(ch), site, ban_types[ban_node->type]);
...
ACMD(do_unban)
{
  ...
  nmudlog(GOD_LOG, LVL_GOD, TRUE, "%s removed the %s-player ban on %s", GET_NAME(ch), ban_types[ban_node->type], ban_node->site);
...
ACMD(do_badname)
{
  ...
  *arg = '\0';
  ...
  if (!(fp = fopen(XNAME_FILE, "a"))) {
    send_to_char(ch,"\n&RERROR: Problems appending to XNAME File&n\r\n");
    return;
  }
  ...
  fprintf(fp, "%s\n", buf);
  fclose(fp);
  sprintf(buf1, "%s has been badnamed.\r\n", buf);
  send_to_char(ch, "&Y%s&n", buf1);
  read_invalid_list();
}
