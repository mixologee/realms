...
//  {3099, 0, 0, LVL_GOD, LIB_ETC "board.mortal", 0},
  {6000, LVL_GOD, LVL_GOD, LVL_GRGOD, LIB_ETC "board.immortal", 0},
//  {3097, LVL_GOD, LVL_GRGOD, LVL_IMPL, LIB_ETC "board.freeze", 0},
//  {3096, 0, 0, LVL_GOD, LIB_ETC "board.social", 0},
...
int board_show_board(int board_type, struct char_data *ch, char *arg, struct obj_data *board)
{
  ...
  if (!num_of_msgs[board_type])
    send_to_char(ch, 
        "                          &CKuvia Bulletin Board\r\n"
        "                      &CFor assistance, type HELP BOARD\r\n"
        "                &WUsage: READ/REMOVE <messg #>, WRITE <header>.\r\n"
        "&B[&CPost&B][       &CAuthor&B][&CSubject                                   &B][   &CDate   &B]\r\n"
        "\r\n"
        "&c-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-&n\r\n");	
  else {
    size_t len = 0;
    ...
    len = snprintf(buf, sizeof(buf),
        ...
        "                      &CFor assistance, type HELP BOARD\r\n"
        "                &WUsage: READ/REMOVE <messg #>, WRITE <header>.\r\n"
        "&B[&CPost&B][   &CDate   &B][       &CAuthor&B][&CSubject                                   &B]\r\n"
        "\r\n"
        "&c-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-&n\r\n");
