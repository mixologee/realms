...
bool build_mail_index(void)
{
  ...
  for (i = 0; i < rec_count; i++) {
    ...
    while (line && *line && *line == '#') get_line(ml_index, line);  /* Skip comments */
...
bool save_mail(struct mail_data *ml)
{
  ...
  if (!(fl = fopen(fname, "w"))) {
    nmudlog(MISC_LOG, LVL_GOD, TRUE, "SYSERR: Couldn't open mail file %s for writing", fname);
//    mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Couldn't open mail file %s for write", fname);
    /* Remove the index entry on failure */
    if (index_change) delete_mail_index_entry(mail_id);
    return FALSE;
  }
...
bool load_mail(long mail_id, struct mail_data *ml)
{
  ...
  if (!(fl = fopen(fname, "r"))) {
    nmudlog(MISC_LOG, LVL_GOD, TRUE, "SYSERR: Couldn't open mail file %s for reading", fname);
//    mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Couldn't open mail file %s for reading.", fname);
    return FALSE;
  }
...
void give_mail_attachments(struct char_data *ch, struct mail_data *ml)
{
  ...
  log("Starting give mail attachment.");

  /* First, do the object attachments */
  while (ml->attachment) {
    obj = ml->attachment;
    log("Mailed Object exists");
    obj_from_mail(obj);
    log("Mailed Object - %d  %s", GET_OBJ_VNUM(obj), obj->short_description);
    obj_to_char(obj, ch);
    log("Object given to user.");
  }

  /* Then the gold */
  if (ml->coins) {
    GET_GOLD(ch) += ml->coins;
    log("Gold given to user.");
  }
}
  ...
  for (i=0; i<=top_of_p_table; i++) {
    ...
    for (j=0; j<=NUM_MAIL_GROUPS; j++) {
      if (to_who[j] == TRUE) {
        ...
        if (mail_groups[j].recipient == MAIL_TO_IMMS && player_table[i].level >= LVL_IMMORT) do_this = TRUE;
	  }
    }
...
ACMD(do_mail)
{
  ...
  if (GET_LEVEL(ch) < CONFIG_MIN_MAIL_ANYWHERE) {
...
/*    if ((mailman = find_immortal_mailman()) == NULL) {
      mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Immortal mailman not found in room %d", MAIL_IMMORTAL_ROOM);
      if (!ROOM_FLAGGED(IN_ROOM(ch), ROOM_MAIL)) {
  ...
  if (GET_LEVEL(ch) < CONFIG_MIN_MAIL_LEVEL) {
...
//    return;
    ...
    if (CONFIG_STAMP_COST > 0 && GET_GOLD(ch) < CONFIG_STAMP_COST && GET_LEVEL(ch) < CONFIG_FREE_MAIL_LEVEL) {
    ...
    if (CONFIG_STAMP_COST > 0 &&  GET_LEVEL(ch) < CONFIG_FREE_MAIL_LEVEL) {
