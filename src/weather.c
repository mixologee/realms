	...
	for (y = 0; y < MAP_WEATH_Y; y++)
	{
		for (x = 0; x < MAP_WEATH_X; x++)
		{
			...
			case AUTUMN:
				...
				else							cond->temp += rand_number(-3, 3);

				// humidity
				if (cond->humidity > 40)		cond->humidity -= 3;
				else							cond->humidity += rand_number(-3, 2);
			...
			if (cond->free_energy > 20000)
				cond->windspeed		+= rand_number(-10, -1);
			cond->windspeed			= URANGE(0, cond->windspeed, 200);

			cond->humidity			= URANGE(0, cond->humidity, 100);
			
			cond->pressure_change	+= rand_number(-3, 3);
			...
			if (old_precip == 0)
			{
				...
				{
					cond->precip_rate += rand_number(1, 10);
					if (cond->temp > 0)
			...
			else
			{
				...
				if (cond->pressure >= pbreak)
				{
					if (cond->windspeed > 50)
						pchange += rand_number(-3, 1);
					else if (cond->windspeed > 20)
						pchange += rand_number(-2, 2);
					else
						pchange += rand_number(-1, 2);

					// check humidity
					if (cond->humidity < 30)
						pchange += rand_number(-4, 2);
					else if (cond->humidity < 60)
						pchange += rand_number(-3, 2);
					else
						pchange += rand_number(-2, 2);
				}
				// increase rate
				...
				{
					if (cond->windspeed > 50)
						pchange += rand_number(-2, 1);
					else if (cond->windspeed > 20)
						pchange += rand_number(-1, 1);
					else
						pchange += rand_number(0, 2);
					...
					if (cond->humidity < 30)
						...
						pchange += rand_number(-1, 3);
				}

				/* energy adjustment */
				if (cond->free_energy > 10000)
					pchange += rand_number(-3, 4);
				else
					pchange += rand_number(-4, 2);
...
void WriteWeatherInfo(void)
{
	...
	for (y = 0; y < MAP_WEATH_Y; y++)
	{
		for (x = 0; x < MAP_WEATH_X; x++)
		{
			...
			fprintf(fp, "Temp: %2ld  Humidity: %2ld  Pressure: %4ld  ",
				cond->temp, cond->humidity, cond->pressure);
			fprintf(fp, "Windspeed: %3ld  Precip Rate: %3ld  ",
				cond->windspeed, cond->precip_rate);
			fprintf(fp, "Pressure change: %2ld  Precip change: %3ld  FEnergy: %ld",
...
void lose_blood() {
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    if(GET_PKBLOOD(d->character) > 0) {
       GET_PKBLOOD(d->character)--;
       if(GET_PKBLOOD(d->character) == 0)
	REMOVE_BIT_AR(PLR_FLAGS(d->character), PLR_KILLER);	
    }
  }
}
