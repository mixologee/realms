...
ACMD(do_survey)
{
	...
	{
		send_to_char(ch, "You can use survey only in the wilderness.\r\n");
		return;
	}
...
/*	if (ON_DECK(ch))
	...
	{
		FERRY_DATA *pFerry = get_ferry(world[IN_ROOM(ch)].extra_data->vnum);

		ry = GET_RY(pFerry->in_room->number);
		rx = GET_RX(pFerry->in_room->number);
	}
	else
	{
		ry = GET_Y(ch);
...
int wild_track( CHAR_DATA *ch, CHAR_DATA *vict, bool silent )
{
	...
	if ( !ch || !vict )
	   return (NOWHERE);
        ...
        dist = distance(GET_X(ch), GET_Y(ch), GET_X(vict), GET_Y(vict));
...
int get_sail_dir( SHIP_DATA *ship, COORD_DATA *cto, bool silent )
{
	...
	iDist = (int) distance( GET_RX(ship->in_room->number), GET_RY(ship->in_room->number), cto->x, cto->y );
	...
	if (iDist > 50)
	{
		...
		if ( !silent && ship->helmperson )
			send_to_char(ship->helmperson, "You fail to follow the course.\r\n");
		return (NOWHERE);
	}
	

	iAngle = calc_angle( GET_RX(ship->in_room->number), GET_RY(ship->in_room->number), cto->x, cto->y, &iDist );
...
void wild_scan(CHAR_DATA *ch)
{
	...
	{
		WEATHER_DATA *sky = get_coord_weather(world[IN_ROOM(ch)].coord);
	...
	if (count == 0)
		send_to_char(ch,"You don't see anyone nearby.\r\n");
}
...
void show_travel( CHAR_DATA *ch )
{
	...
	if (!world[IN_ROOM(ch)].portal_stone )
	{
		send_to_char(ch, "You can't Travel if in room there isn't a portal stone.\r\n");
		return;
	}

	ch_printf(ch, "From the portal stone '&b&1%s&0' you can Travel to:\r\n",
		world[IN_ROOM(ch)].portal_stone->name);
	for ( pn = 0; pn < MAX_LNK_STONE; pn++ )
	{
		if ( world[IN_ROOM(ch)].portal_stone->link_to[pn] )
		{
			PSTONE_DATA *pstone = get_pstone(world[IN_ROOM(ch)].portal_stone->link_to[pn]);
...
void stone_travel( CHAR_DATA *ch, char *argument )
{
	...
	for ( pn = 0; pn < MAX_LNK_STONE; pn++ )
	{
		if ( world[IN_ROOM(ch)].portal_stone->link_to[pn] == pstone->vnum )
			break;
	}
...
ACMD(do_camp)
{
	ROOM_AFFECT *raff;
	int move_needed = 100;
	ROOM_DATA *pRoom = &world[ch->in_room];

        if (!ROOM_FLAGGED(IN_ROOM(ch), ROOM_WILDERNESS))
//	if (!IS_FLAGGED(IN_ROOM(ch), ROOM_WILDERNESS)) //!IN_WILD(ch))
	{
		send_to_char(ch, "You can make a campsite only in the wilderness.\r\n");
		return;
	}
	
	if ( RIDING(ch) )
	{
		send_to_char(ch, "You cannot prepare a camp while mounted.\r\n");
		return;
	}

	if ( WAGONER(ch) )
	{
		send_to_char(ch, "You cannot prepare a camp while driving a vehicle.\r\n");
		return;
	}

	if (ch->action)
	{
		send_to_char(ch, "You are too busy to do this!\r\n");
		return;
	}
	
	if ((raff = get_room_aff_bitv(pRoom, RAFF_SHELTER)))
	{
		ROOM_DATA *tRoom = pRoom;

		/* refresh the current campsite */
		raff->timer = 60;
		send_to_char(ch, "You throw more wood on the fire and get the campsite ready.\r\n");
		if (!GET_INVIS_LEV(ch))
	...
	else
	{
		...
		ce->ch		= ch;
		ce->pRoom	= &world[ch->in_room];

		ch->action = event_create( camp_event, ce, 100 );

		send_to_char(ch, "You begin to prepare a campsite...\r\n");
...
EVENTFUNC(camp_event)
{
	...
	if (!ROOM_FLAGGED(tRoom->number, ROOM_WILDERNESS))
	{
		send_to_char(ch, "You can prepare campsites only in the wilderness.\r\n");
		return (0);
	}
	...
	if ( tRoom->number != GET_ROOM_VNUM(ch->in_room) )
	{
		send_to_char(ch, "You are no longer near where you began the campsite.\r\n");
		return (0);
	}
	
	send_to_char(ch, "You complete your campsite, now you can leave this world for awhile.\r\n");
	if (!GET_INVIS_LEV(ch))
		act("$n camps, preparing to leave this world.", TRUE, ch, 0, 0, TO_ROOM);

	free(event_obj);
	
	if ((raff = get_room_aff_bitv(tRoom, RAFF_SHELTER)))
		raff->timer			= 60;
	else
	{
		...
		raff->coord->y		= GET_RY(tRoom->number);
		raff->coord->x		= GET_RX(tRoom->number);
