...
int objsave_save_obj_record(struct obj_data *obj, FILE *fp, int locate)
{
  ...
  if (GET_OBJ_VNUM(obj) != NOTHING)
    temp=read_object(GET_OBJ_VNUM(obj), REAL);
  else {
    temp = create_obj();
...
void Crash_listrent(struct char_data *ch, char *name)
{
  ...
  default:
	...
	for (current = loaded; current != NULL; current=current->next)
	  len += snprintf(buf+len, sizeof(buf)-len, "[%5ld] (%5d) %-20s\r\n",
    ...
    obj_data *obj, long *cost, long *nitems, int display, int factor)
{
  ...
  if (obj) {
    if (!Crash_is_unrentable(obj)) {
      (*nitems)++;
      *cost += MAX(0, (GET_OBJ_COND(obj) * factor));
      if (display) {
...
obj_save_data *objsave_parse_objects(FILE *fl)
{
  ...
  while (TRUE) {
	...
	/* Go read next line - nothing more to see here. */
          ...
          log("Nonexistent object %ld found in rent file.", nr);
        }
      }
      /* go read next line - nothing more to see here. */
      continue;
