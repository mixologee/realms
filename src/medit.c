...
ACMD(do_oasis_medit)
{
  ...
  if (d->olc) {
    nmudlog(SYS_LOG, LVL_GOD, TRUE, "SYSERR: do_oasis_medit: player already had olc structure");
//    mudlog(BRF, LVL_IMMORT, TRUE,
 //     "SYSERR: do_oasis_medit: Player already had olc structure.");
  ...
  if (save) {
    ...
    nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s saves mob information for zone %d", GET_NAME(ch), zone_table[OLC_ZNUM(d)].number);
...
//      GET_NAME(ch), zone_table[OLC_ZNUM(d)].number);
  ...
  nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s starts editing zone %d : Allowed zone %d", GET_NAME(ch), zone_table[OLC_ZNUM(d)].number, GET_OLC_ZONE(ch));
...
void medit_disp_menu(struct descriptor_data *d)
{
  ...
  "%sC%s) Num HP Dice: [%s%4ld%s],  %sD%s) Size HP Dice: [%s%4ld%s],  %sE%s) HP Bonus: [%s%5ld%s]\r\n"
  ...
  write_to_output(d,
	  ...
	  grn, nrm, cyn, flags,
	  grn, nrm, cyn, flag2,
          grn, nrm, yel, mclass[(int)GET_MCLASS(mob)],
...
void medit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case MEDIT_CONFIRM_SAVESTRING:
    ...
    case 'Y':
      ...
      nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s edits mob %d", GET_NAME(d->character), OLC_NUM(d));
...
//      break; 
  ...
  case MEDIT_D_DESC:
     ...
     */
    cleanup_olc(d, CLEANUP_ALL);
    nmudlog(SYS_LOG, LVL_BUILDER, TRUE, "SYSERR: medit_parse() reached D_DESC case!");
...
/* Numerical responses. */
  ...
  case MEDIT_HITROLL:
    GET_HITROLL(OLC_MOB(d)) = LIMIT(i, 0, 250);
    break;

  case MEDIT_DAMROLL:
    GET_DAMROLL(OLC_MOB(d)) = LIMIT(i, 0, 250);
    break;

  case MEDIT_NDD:
    GET_NDD(OLC_MOB(d)) = LIMIT(i, 0, 250);
    break;
  ...
  case MEDIT_ADD_HP:
    GET_MOVE(OLC_MOB(d)) = LIMIT(i, 0, 300000);
    break;
  ...
  case MEDIT_MCLASS:
    GET_MCLASS(OLC_MOB(d)) = LIMIT(i, 0, NUM_MCLASSES - 1);
    break;

  case MEDIT_LEVEL:
    GET_LEVEL(OLC_MOB(d)) = LIMIT(i, 1, 50);
    {
  ...
  default:
    /* We should never get here. */
    cleanup_olc(d, CLEANUP_ALL);
    nmudlog(SYS_LOG, LVL_BUILDER, TRUE, "SYSERR: medit_parse() reached default case!");
