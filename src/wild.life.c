...
void encounter(ROOM_DATA *pRoom)
{
	...
	{
		if (pLife->sect == pRoom->sector_type)
			if (rand_number(0, 100) < percentage((use_custom_life ? pLife->num_curr : pLife->num_max), tot))
				break;
	}
...
ACMD(do_life)
{
	...
	nsect = atoi(arg);
	if (nsect < 0 || nsect > MAX_SECT_TYPES)
	{
