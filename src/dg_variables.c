...
char *skill_percent(struct char_data *ch, char *skill)
{
  ...
  skillnum = find_skill_num(skill, 0, strlen(skill),spell_check, 0);
  skillnum++;
                   ...
                   char *str, size_t slen)
{
  ...
  if (!str_cmp(field, "strlen")) {                     /* strlen    */
    snprintf(str, slen, "%ld", strlen(vd->value));
    return TRUE;
    ...
    *str = '\0';
    ...
    int c = atoi(subfield);
    snprintf(str, slen, "%s", wordat(vd->value, c));
    return TRUE;
 ...
 * Addition inspired by Jamie Nelson */
	...
	snprintf(str, slen, "%d", GET_ALIGNMENT(c));
            ...
            if (subfield && *subfield) {
              int spell = find_skill_num(subfield, 0, strlen(subfield),spell_check, 0);
              spell++;
        ...
        case 'h':
          if (!str_cmp(field, "has_item")) {
            ...
            snprintf(str, slen, "%ld", GET_HIT(c));
          }
        ...
        case 'i':
          ...
          else if (!str_cmp(field, "is_preceptor")) {
            if (subfield && *subfield) {
              if (!str_cmp("on", subfield))
                SET_BIT_AR(PLR_FLAGS(c), PLR_PRECEPTOR);
              else if (!str_cmp("off", subfield))
                REMOVE_BIT_AR(PLR_FLAGS(c), PLR_PRECEPTOR);
            }
            if (PLR_FLAGGED(c, PLR_PRECEPTOR))
              strcpy(str, "1");
            else
              strcpy(str, "0");
          }
        ...
        case 'm':
          if (!str_cmp(field, "maxhitp")) {
            ...
            snprintf(str, slen, "%ld", GET_MAX_HIT(c));
          }
            ...
            snprintf(str, slen, "%ld", GET_MANA(c));
          }
            ...
            snprintf(str, slen, "%ld", GET_MAX_MANA(c));
          }
            ...
            snprintf(str, slen, "%ld", GET_MOVE(c));
          }
            ...
            snprintf(str, slen, "%ld", GET_MAX_MOVE(c));
          }
...
/* see note in dg_scripts.h */
          ...
          if (!str_cmp(field, "sex"))
            ...
            if (!IS_NPC(c) && subfield && *subfield) {
              ...
              if (amount && *amount && is_number(amount)) {
                int skillnum = find_skill_num(skillname, 0, strlen(skillname),spell_check, 0);
                skillnum++;
                if (skillnum > 0) {
            ...
            *str = '\0'; /* so the parser know we recognize 'skillset' as a field */
            ...
            snprintf(str, slen, "%ld", GET_SAVE(c, SAVING_PARA));
          }
            ...
            snprintf(str, slen, "%ld", GET_SAVE(c, SAVING_ROD));
          }
            ...
            snprintf(str, slen, "%ld", GET_SAVE(c, SAVING_PETRI));
          }
            ...
            snprintf(str, slen, "%ld", GET_SAVE(c, SAVING_BREATH));
          }
            ...
            snprintf(str, slen, "%ld", GET_SAVE(c, SAVING_SPELL));
          }
	...
	case 'v':
          if (!str_cmp(field, "vnum")) {
            if (subfield && *subfield) {
              snprintf(str, slen, "%d", IS_NPC(c) ? (GET_MOB_VNUM(c) == atoi(subfield)) : -1 );
            } else {
              if (IS_NPC(c))
	...
	case 'c':
          ...
          else if (!str_cmp(field, "condition")) {
            if (subfield && *subfield) {
              int addition = atoi(subfield);
              GET_OBJ_COND(o) = MAX(1, addition + GET_OBJ_COND(o));
            }
            snprintf(str, slen, "%d", GET_OBJ_COND(o));
          }
...
o->contains) ? "1" : "0"));
        ...
        case 'i':
          if (!str_cmp(field, "id"))
            ...
            if (IN_ROOM(o) != NOWHERE)
              snprintf(str, slen,"%c%ld",UID_CHAR, (long int)world[IN_ROOM(o)].number + ROOM_ID_BASE);
            else
              *str = '\0';
          }
          else if (!str_cmp(field, "is_pc")) { 
	  ...
	  break;
        ...
        case 'v':
          if (!str_cmp(field, "vnum"))
            if (subfield && *subfield) {
              snprintf(str, slen, "%ld", (long int)(GET_OBJ_VNUM(o) == atoi(subfield)));
            } else {
      ...
      if (r->number == 0) {
        ...
        if (subfield && *subfield) {
          snprintf(str, slen, "%ld", (long int)(r->number == atoi(subfield)));
        } else {
