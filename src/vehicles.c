...
VEHICLE_DATA *find_vehicle_in_room_by_name( CHAR_DATA *ch, char *name )
{
	...
	for ( pVeh = world[IN_ROOM(ch)].vehicles; pVeh; pVeh = pVeh->next_in_room )
	{
		if ( isname( name, pVeh->name ) || !str_cmp( name, "vehicle" ) )
...
void char_enter_vehicle( CHAR_DATA *ch, VEHICLE_DATA *vehicle )
{
	...
	if ( RIDING(ch) )
	{
		send_to_char(ch, "You cannot enter a vehicle while mounted.\r\n");
		return;
	}

	if ( WAGONER(ch) )
	{
		send_to_char(ch, "You cannot enter a vehicle while driving a vehicle.\r\n");
		return;
	}

	if ( ch->in_vehicle )
	{
		send_to_char(ch, "You are already inside a vehicle.\r\n");
		return;
	}

	if (IS_SET(vehicle->flags, VEH_DESTROYED))
	{
		send_to_char(ch, "You cannot enter a destroyed vehicle.\r\n");
		return;
	}
...
void start_be_wagoner( CHAR_DATA *ch, VEHICLE_DATA *vehicle )
{
	...
	if ( RIDING(ch) )
	{
		send_to_char(ch, "You cannot drive a vehicle while mounted.\r\n");
		return;
	}

	if (IS_SET(vehicle->flags, VEH_DESTROYED))
	{
		send_to_char(ch, "You cannot drive a destroyed vehicle.\r\n");
		return;
	}
...
void vehicle_to_room( VEHICLE_DATA *pVeh, ROOM_DATA *pRoom )
{
	...
	if ( IS_WILD(pRoom) )
	{
		...
		if (ROOM_FLAGGED(pRoom->number, ROOM_WILD_REMOVE))
...
bool vehicle_can_go( VEHICLE_DATA *pVeh, ROOM_DATA *to_room )
{
	 ...
	 * in do_simple_move()
	 */
	if ( ROOM_FLAGGED(to_room->number, ROOM_DEATH) )
		return (FALSE);
	...
	if ( to_room->sector_type == SECT_ROAD		|| 
	     to_room->sector_type == SECT_PLAINS	||
...
OBJ_DATA *obj_to_vehicle( OBJ_DATA *obj, VEHICLE_DATA *vehicle )
{
	...
	vehicle->curr_val.capacity += GET_OBJ_WEIGHT(obj);
...
void obj_from_vehicle( OBJ_DATA *obj )
{
	...
	obj->in_vehicle->curr_val.capacity -= GET_OBJ_WEIGHT(obj);
...
void vehicle_load_goods( CHAR_DATA *ch, OBJ_DATA *goods, VEHICLE_DATA *vehicle )
{
	...
	if (IS_SET(vehicle->flags, VEH_DESTROYED))
	{
		send_to_char(ch, "You cannot put items in a destroyed vehicle.\r\n");
		return;
	}

	if ( vehicle->curr_val.capacity + GET_OBJ_WEIGHT(goods) > vehicle->max_val.capacity )
	{
...
void stat_vehicle(CHAR_DATA *ch, VEHICLE_DATA *vehicle)
{
	...
	sprintbit(vehicle->flags, veh_flags_descr, dflags, sizeof(dflags));
	sprintf(sbaf,
		"%s%s\r\n"
		"Flags    : %s\r\n"
		"Capacity : %s (%5d/%5d)\r\n"
...
ACMD(do_newvehicle)
{
	...
	if ( !*arg )
	{
		send_to_char(ch, "Possible vehicles are:\r\n");
		...
		send_to_char(ch, "Which type of vehicle?\r\n");
		return;
	}
...
ACMD(do_out)
{
	...
	if ( !ch->in_vehicle )
	{
		send_to_char(ch, "You are not inside a vehicle.\r\n");
		return;
	}
...
ACMD(do_drive)
{
	...
	if ( !str_cmp(arg1, "stop") )
	{
		...
		{
			send_to_char(ch, "You are not driving any vehicle.\r\n");
			return;
		}
...
ACMD(do_yoke)
{
	...
	if ( RIDING(ch) )
	{
		send_to_char(ch, "You cannot yoke draft animal to a vehicle while mounted.\r\n");
		return;
	}

	if ( WAGONER(ch) )
	{
		send_to_char(ch, "You cannot yoke draft animal to a vehicle while driving the vehicle.\r\n");
		return;
	}
	...
	if ( !*arg1 || !*arg2 )
	{
		send_to_char(ch, "Usage: yoke <mob> <vehicle>\r\n");
		return;
	}
	...
	{
		send_to_char(ch, "You can yoke only draft animal to a vehicle.\r\n");
		return;
	}
	...
	if (IS_SET(vehicle->flags, VEH_DESTROYED))
	{
		send_to_char(ch, "You cannot yoke animals to a destroyed vehicle.\r\n");
		return;
	}
...
ACMD(do_unyoke)
{
	...
	if ( RIDING(ch) )
	{
		send_to_char(ch, "You cannot unyoke draft animal from a vehicle while mounted.\r\n");
		return;
	}

	if ( WAGONER(ch) )
	{
		send_to_char(ch, "You cannot unyoke draft animal from a vehicle while driving the vehicle.\r\n");
		return;
	}
	...
	if ( !*arg1 )
	{
		send_to_char(ch, "Usage: unyoke <mob>\r\n");
		return;
	}
	...
	if ( !IS_NPC(mob) || !MOB_FLAGGED(mob, MOB_DRAFT_ANIMAL))
	{
		send_to_char(ch, "You can unyoke only draft animal from a vehicle.\r\n");
		return;
	}
	...
	if ( !mob->mob_specials.yoke )
	{
		...
		send_to_char(ch, "There is a problem, call an Immortal.\r\n");
		return;
	}

	if (IS_SET(vehicle->flags, VEH_DESTROYED))
	{
		send_to_char(ch, "You cannot unyoke animals from a destroyed vehicle.\r\n");
		return;
	}
...
void hit_vehicle(CHAR_DATA *ch, VEHICLE_DATA *pVeh)
{
	...
	if (GET_OBJ_TYPE(weapon) != (ITEM_WEAPON || ITEM_BOW || ITEM_CROSSBOW || ITEM_SLING))
	{
