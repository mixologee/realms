...
void dg_read_trigger(FILE *fp, void *proto, int type)
{
  ...
  if (count != 2) {
    nmudlog(SYS_LOG, LVL_BUILDER, TRUE, "SYSERR: Error assigning trigger - Line was %s", line);
//    mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: Error assigning trigger! - Line was\n  %s", line);
  ...
  if (rnum == NOTHING) {
    switch(type) {
      case MOB_TRIGGER:
        nmudlog(SYS_LOG, LVL_BUILDER, TRUE, "SYSERR: dg_read_trigger: trigger vnum %d asked for but it doesn't exist (mob: %s - %d)",
...
//               vnum, GET_NAME((char_data *)proto), GET_MOB_VNUM((char_data *)proto));
        break;
      case WLD_TRIGGER:
        nmudlog(SYS_LOG, LVL_BUILDER, TRUE, "SYSERR: dg_read_trigger: trigger vnum %d asked for but it doesn't exist (room: %d)",
...
//               vnum, GET_ROOM_VNUM( ((room_data *)proto)->number ));
        break;
      default:
        nmudlog(SYS_LOG, LVL_BUILDER, TRUE, "SYSERR: dg_read_trigger: trigger vnum %d asked for but doesn't exist (default case error)", vnum);
//        mudlog(BRF, LVL_BUILDER, TRUE,
  ...
  switch(type) {
    ...
    case WLD_TRIGGER:
      ...
      if (rnum != NOTHING) {
        ...
        nmudlog(SYS_LOG, LVL_BUILDER, TRUE, "SYSERR: non-existant trigger %d assigned to room %d", vnum, room->number);
//        mudlog(BRF, LVL_BUILDER, TRUE,
...
//               vnum, room->number);
      ...
      break;
    default:
      nmudlog(SYS_LOG, LVL_BUILDER, TRUE, "SYSERR: trigger vnum %d assigned to non-mob/obj/room (default case)", vnum);
//      mudlog(BRF, LVL_BUILDER, TRUE,
...
void dg_obj_trigger(char *line, struct obj_data *obj)
{
  ...
  if (count != 2) {
    nmudlog(SYS_LOG, LVL_BUILDER, TRUE, "SYSERR: dg_obj_trigger(): error assigning trigger - line was %s", line);
//    mudlog(BRF, LVL_BUILDER, TRUE,
//           "SYSERR: dg_obj_trigger() : Error assigning trigger! - Line was:\n  %s", line);
  ...
  rnum = real_trigger(vnum);
  if (rnum==NOTHING) {
    nmudlog(SYS_LOG, LVL_BUILDER, TRUE, "SYSERR: trigger vnum %d asked for but doesn't exist (object: %s - %d)", vnum, obj->short_description, GET_OBJ_VNUM(obj));
//    mudlog(BRF, LVL_BUILDER, TRUE,
  ...
  switch (type)
  {
    case MOB_TRIGGER:
      ...
      while (trg_proto) {
        ...
        if (rnum==NOTHING) {
          nmudlog(SYS_LOG, LVL_BUILDER, TRUE, "SYSERR: trigger %d doesn't exist for mob %d", trg_proto->vnum, mob_index[mob->nr].vnum);
//          mudlog(BRF, LVL_BUILDER, TRUE,
...
//                 trg_proto->vnum, mob_index[mob->nr].vnum);
    ...
    case WLD_TRIGGER:
      ...
      while (trg_proto) {
        rnum = real_trigger(trg_proto->vnum);
        if (rnum==NOTHING) {
          nmudlog(SYS_LOG, LVL_BUILDER, TRUE, "SYSERR: trigger %d doesn't exist for room %d", trg_proto->vnum, room->number);
//          mudlog(BRF, LVL_BUILDER, TRUE,
...
//                 trg_proto->vnum, room->number);
      ...
      break;
    default:
      nmudlog(SYS_LOG, LVL_BUILDER, TRUE, "SYSERR: unknown type for assign_triggers() (default case)");
//      mudlog(BRF, LVL_BUILDER, TRUE,
