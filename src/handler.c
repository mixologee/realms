...
struct char_data *get_msg_player(char *name)
{
 ...
 for(ch = character_list; ch; ch = ch->next)             
        if (!IS_NPC(ch) && isname(name, GET_NAME(ch)))
                return(ch);
...
void split_obj( OBJ_DATA *obj, int num )
{
        ...
        {
                LINK(rest, world[obj->in_room].first_content, world[obj->in_room].last_content,
                        next_content, prev_content);
...
/*
 *  BJ_DATA *clone_object( OBJ_DATA *obj )
...
OBJ_DATA *clone_object( OBJ_DATA *obj )
{
        ...
        GET_OBJ_PERM(clone)[0]                             = GET_OBJ_PERM(obj)[0];
        GET_OBJ_PERM(clone)[1]                             = GET_OBJ_PERM(obj)[1];
        GET_OBJ_PERM(clone)[2]                             = GET_OBJ_PERM(obj)[2];
        GET_OBJ_PERM(clone)[3]                             = GET_OBJ_PERM(obj)[3];

        GET_OBJ_TYPE(clone)                             = GET_OBJ_TYPE(obj);

        GET_OBJ_EXTRA(clone)[0]                    = GET_OBJ_EXTRA(obj)[0];
        GET_OBJ_EXTRA(clone)[1]                    = GET_OBJ_EXTRA(obj)[1];
        GET_OBJ_EXTRA(clone)[2]                    = GET_OBJ_EXTRA(obj)[2];
        GET_OBJ_EXTRA(clone)[3]                    = GET_OBJ_EXTRA(obj)[3];

        GET_OBJ_WEAR(clone)[0]                             = GET_OBJ_WEAR(obj)[0];
        GET_OBJ_WEAR(clone)[1]                             = GET_OBJ_WEAR(obj)[1];
        GET_OBJ_WEAR(clone)[2]                             = GET_OBJ_WEAR(obj)[2];
        GET_OBJ_WEAR(clone)[3]                             = GET_OBJ_WEAR(obj)[3];
...
CHAR_DATA *get_char_elsewhere_vis(CHAR_DATA *ch, ROOM_DATA *pRoom, char *name, int *numbr)
{
        ...
        for (i = world[ch->in_room].people; i && *numbr; i = i->next_in_room)
        {
