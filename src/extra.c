...
zone_rnum create_new_zone(zone_vnum vzone_num, room_vnum bottom, room_vnum top, const char **error)
{
  ...
  if (!(fp = fopen(buf, "w"))) {
    nmudlog(SYS_INFO, LVL_IMPL, TRUE, "SYSERR: OLC can't write new zone file!!");
//    mudlog(BRF, LVL_IMPL, TRUE, "SYSERR: OLC: Can't write new zone file.");
  ...
  if (!(fp = fopen(buf, "w"))) {
    nmudlog(SYS_INFO, LVL_IMPL, TRUE, "SYSERR: OLC can't write new world file!!");
//    mudlog(BRF, LVL_IMPL, TRUE, "SYSERR: OLC: Can't write new world file.");
  ...
  if (!(fp = fopen(buf, "w"))) {
    nmudlog(SYS_LOG, LVL_IMPL, TRUE, "SYSERR: OLC can't write new mob file!!");
//    mudlog(BRF, LVL_IMPL, TRUE, "SYSERR: OLC: Can't write new mob file.");
  ...
  if (!(fp = fopen(buf, "w"))) {
    nmudlog(SYS_LOG, LVL_IMPL, TRUE, "SYSERR: OLC can't write new obj file!!");
//    mudlog(BRF, LVL_IMPL, TRUE, "SYSERR: OLC: Can't write new obj file.");
  ...
  if (!(fp = fopen(buf, "w"))) {
    nmudlog(SYS_LOG, LVL_IMPL, TRUE, "SYSERR: OLC can't write new shop file!!");
//    mudlog(BRF, LVL_IMPL, TRUE, "SYSERR: OLC: Can't write new shop file.");
  ...
  if (!(fp = fopen(buf, "w"))) {
    nmudlog(SYS_LOG, LVL_IMPL, TRUE, "SYSERR: OLC can't write new trigger file!!");
//    mudlog(BRF, LVL_IMPL, TRUE, "SYSERR: OLC: Can't write new trigger file");
  ...
  snprintf(buf, sizeof(buf), "%s/%ld.qst", QST_PREFIX, vzone_num);
  if (!(fp = fopen(buf, "w"))) {
    nmudlog(SYS_LOG, LVL_IMPL, TRUE, "SYSERR: OLC can't write new quest file!!");
...
void create_world_index(int znum, const char *type)
{
  ...
  if (!(oldfile = fopen(old_name, "r"))) {
    nmudlog(SYS_LOG, LVL_IMPL, TRUE, "SYSERR: OLC failed to open %s", old_name);
//    mudlog(BRF, LVL_IMPL, TRUE, "SYSERR: OLC: Failed to open %s.", old_name);
    return;
  } else if (!(newfile = fopen(new_name, "w"))) {
    nmudlog(SYS_LOG, LVL_IMPL, TRUE, "SYSERR: OLC failed to open %s", new_name);
...
#endif
    log("SYSERR: GenOLC: save_zone: Invalid real zone number %ld. (0-%ld)", zone_num, top_of_zone_table);
    return FALSE;
  }

  snprintf(fname, sizeof(fname), "%s/%ld.new", ZON_PREFIX, zone_table[zone_num].number);
  if (!(zfile = fopen(fname, "w"))) {
    nmudlog(SYS_LOG, LVL_BUILDER, TRUE, "SYSERR: OLC SAVE: Can't write zone %ld!!", zone_table[zone_num].number);
//    mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: OLC: save_zones:  Can't write zone %ld.", zone_table[zone_num].number);
    return FALSE;
  }
  ...
  for (subcmd = 0; ZCMD(zone_num, subcmd).command != 'S'; subcmd++) {
    ...
    case '*':
      ...
      continue;
    default:
      nmudlog(SYS_LOG, LVL_BUILDER, TRUE, "SYSERR: OLC: z_save_to_disk() has unkown cmd [%c] - NOT SAVING", ZCMD(zone_num, subcmd).command);
//      mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: OLC: z_save_to_disk(): Unknown cmd '%c' - NOT saving", ZCMD(zone_num, subcmd).command);
  ...
  fputs("S\n$\n", zfile);
  fclose(zfile);
  snprintf(oldname, sizeof(oldname), "%s/%ld.zon", ZON_PREFIX, zone_table[zone_num].number);
  remove(oldname);
