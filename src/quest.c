...
// log("num quests = %d", GET_NUM_QUESTS(ch));
...
void generic_complete_quest(struct char_data *ch)
{
  ...
  if (--GET_QUEST_COUNTER(ch) <= 0) {
    ...
    if (QST_OBJ(rnum) && QST_OBJ(rnum) != -1) {
    ...
    if ((real_quest(QST_NEXT(rnum)) != NOTHING))
      if (QST_NEXT(rnum) != vnum) 
       if(!is_complete(ch, QST_NEXT(rnum))) {
...
void quest_stat(struct char_data *ch, char argument[MAX_STRING_LENGTH])
{
  ...
  if (GET_LEVEL(ch) < LVL_IMMORT)
	...
	"Quit Message:\r\n&c%s&n"
	"Type  : &y%s&n\r\n"
        "Target: &y%ld&n &y%s&n, Quantity: &y%d&n\r\n"
