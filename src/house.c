  ...
  *buf = '\0';
	...
	for (current = loaded; current != NULL; current = current->next)
	  len += snprintf(buf+len, sizeof(buf)-len, " [%5d] (%5dau) %s\r\n",
	    GET_OBJ_VNUM(current->obj), GET_OBJ_COND(current->obj), current->obj->short_description);
...
void hcontrol_pay_house(struct char_data *ch, char *arg)
{
  ...
  if (!*arg)
    ...
    nmudlog(INFO_LOG, LVL_GOD, TRUE, "Payment for house %s collected by %s", arg, GET_NAME(ch));
...
ACMD(do_gohome)
{
   ...
   {
   if ((GET_ROOM_VNUM(IN_ROOM(ch)) < 300 || GET_ROOM_VNUM(IN_ROOM(ch)) > 500) || !FIGHTING(ch) || PLR_FLAGGED(ch, PLR_AFTERLIFE))
    if (GET_IDNUM(ch) == house_control[i].owner && !ROOM_FLAGGED(IN_ROOM(ch), ROOM_NPK) && !ROOM_FLAGGED(IN_ROOM(ch), ROOM_CPK))
    {
    char_from_room(ch);
    send_to_char(ch, "&RYou find yourself sitting at home.&n\r\n");
    snprintf(buf, sizeof(buf), "%d", house_control[i].vnum );
