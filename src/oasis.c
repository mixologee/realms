...
void cleanup_olc(struct descriptor_data *d, byte cleanup_type)
{
  ...
  if (d->character) {
    ...
    if (cleanup_type == CLEANUP_CONFIG)
      nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s stops editing the game configuration", GET_NAME(d->character));
//      mudlog(BRF, LVL_IMMORT, TRUE, "OLC: %s stops editing the game configuration", GET_NAME(d->character));
    else if (STATE(d) == CON_TEDIT)
      nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s stops editing text files", GET_NAME(d->character));
//      mudlog(BRF, LVL_IMMORT, TRUE, "OLC: %s stops editing text files.", GET_NAME(d->character));
    else if (STATE(d) == CON_HEDIT)
      nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s stops editing help files", GET_NAME(d->character));
//      mudlog(CMP, LVL_IMMORT, TRUE, "OLC: %s stops editing help files.", GET_NAME(d->character));
    else
      nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s stops editing zone %d allowed zone %d", GET_NAME(d->character), zone_table[OLC_ZNUM(d)].number, GET_OLC_ZONE(d->character));
...
void send_cannot_edit(struct char_data *ch, zone_vnum zone)
{
  ...
  if (GET_OLC_ZONE(ch) != NOWHERE) {
    ...
    nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s tried to edit zone %d (allowed zone %d)", GET_NAME(ch), zone, GET_OLC_ZONE(ch));
//    sprintf(buf, "OLC: %s tried to edit zone %d (allowed zone %d).", GET_NAME(ch), zone, GET_OLC_ZONE(ch));
    ...
    nmudlog(OLC_LOG, LVL_GOD, TRUE, "%s tried to edit zone %d", GET_NAME(ch), zone);
