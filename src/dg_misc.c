...
int valid_dg_target(struct char_data *ch, int bitvector)
{
  ...
  else if (GET_LEVEL(ch) < LVL_GOD) /* THIS WAS LVL_IMMORT, TAKING MASTERS OUT OF THE GAME */
...
{
  if (GET_LEVEL(vict)>=LVL_GOD && (dam > 0)) {
  ...
  if (GET_POS(vict) == POS_DEAD) {
    if (!IS_NPC(vict))
      nmudlog(INFO_LOG, LVL_GOD, TRUE, "%s killed by script at %s", GET_NAME(vict), world[IN_ROOM(vict)].name);
