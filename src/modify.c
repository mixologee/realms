...
void string_add(struct descriptor_data *d, char *str)
{
  ...
  if (action != STRINGADD_OK)
    ...
    prob = strlen(str) + strlen(*d->str) + 3;
...
ACMD(do_skillset)
{
  ...
  help[qend - 1] = '\0';
  if (((skill) = find_skill_num(help, 0, strlen(help),spell_check, 0) + 1) <= 0) {
...
void page_string(struct descriptor_data *d, char *str, int keep_internal)
{
  ...
  if (!str || !*str)
   ...
   if ((GET_PAGE_LENGTH(d->character) < 5 || GET_PAGE_LENGTH(d->character) > 528))
...
void show_string(struct descriptor_data *d, char *input)
{
  ...
  if (d->showstr_page + 1 >= d->showstr_count) {
    send_to_char(d->character, "\r\n%s&n", d->showstr_vector[d->showstr_page]);
    free(d->showstr_vector);
    d->showstr_vector = NULL;
    d->showstr_count = 0;
    if (d->showstr_head) {
