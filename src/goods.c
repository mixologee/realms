...
int bsearch_goods_exact(const char *name, int first, int top)
{
	...
	{
		sn = first;
		log("sn = %d, first = %d, top = %d", sn, first, top);
		log("good name = %s", goods_table[sn]->name);
		if (!IS_VALID_GOOD(sn))
		{
			return (-1);
  		}

                if (is_abbrev(name, goods_table[sn]->name))
                {
                        return (sn);
                }
...
OBJ_DATA *create_good_obj(GOOD_DATA *pGood, int amount)
{
	...
	GET_OBJ_QUALITY(obj)	        = rand_number(30, 80);
...
ACMD(do_newgoods)
{
	...
	if (g == NULL)
	{
		send_to_char(ch, "Goods names must be enclosed in the Holy Magic Symbols: '\r\n");
		return;
	}
...
MARKET_DATA *locate_bld_market(BUILDING_DATA *pBld)
{
	...
	if (IS_WILD(pBld->in_room))
	{
		...
		{
			iDist = (int) distance(GET_RX(pBld->in_room->number), GET_RY(pBld->in_room->number), pMk->heart.x, pMk->heart.y);
...
ACMD(do_tp)
{
		...
		*buf = '\0';
		for (pTp = first_trading_post; pTp; pTp = pTp->next)
		{
			...
			if (IS_BUILDING(pTp->in_room))
				...
				sprintf(bloc, "Wilderness [%d %d]", GET_RY(pTp->in_room->number), GET_RX(pTp->in_room->number));
		...
		if (!*buf)
			send_to_char(ch, "No Trading Post found.\r\n");
		else
			page_string(ch->desc, buf, 0);
	...
	if (is_abbrev(arg, "new"))
	{
		...
		if (!IN_BUILDING(ch))
		{
			send_to_char(ch, "You must be in a STORE type building to create a new TP.\r\n");
			return;
		}
		...
		if (pBld->trp)
		{
			send_to_char(ch, "This building already host a Trading Post.\r\n");
			return;
		}
		...
		if (!*arg)
		{
			send_to_char(ch, "You must specify a TP type.\r\nPossible types are:\r\n");
		...
		if (nt >= NUM_TP_TYPES)
		{
			send_to_char(ch, "Unknown TP type.\r\nPossible types are:\r\n");
...
//		pBld->rooms[ch->in_room->number]->func  = tradingpost;
		pBld->rooms[IN_ROOM(ch)]->func	= tradingpost;
	...
	if (!*arg)
	{
		...
		send_to_char(ch, "List of Goods in TP:\r\n");
		*buf = '\0';
		for (tGood = pTp->first_tpgood; tGood; tGood = tGood->next)
		{
...
//	{
//		send_to_char(ch, "Valid commands for TP are: add, free\r\n");
//		return;
//	}
	...
	if (is_abbrev(arg, "add"))
	{
		...
		{
			send_to_char(ch, "Good already present in TP.\r\n");
			return;
		}
		...
		send_to_char(ch, "Good succesfully added.\r\n");
		return;
	}
	...
	if (is_abbrev(arg, "remove"))
	{
		...
		send_to_char(ch, "Good succesfully removed.\r\n");
		return;
	}
}
...
ACMD(do_market)
{
	...
	if (!str_cmp(arg, "info"))
	{
		...
		if (newspaper)
			...
			send_to_char(ch, "Trade Newspaper not present.\r\n");
		return;
	}

	if (!str_cmp(arg, "list"))
	{
		send_to_char(ch, "Current markets in game:\r\n");
		for (pMk = first_market; pMk; pMk = pMk->next)
		{
			if (!pMk->size)
				continue;
	...
	if (pMk->affect)
	{
		...
		for (pAff = pMk->affect; pAff; pAff = pAff->next)
		{
			sprintbit(pAff->bitvector, mk_aff_descr, dflags, sizeof(dflags));
			sprinttype(pAff->what, mk_aff_what, swhat, sizeof(swhat));
...
ACMD(do_mkaff)
{
	...
	if (!*arg)
	{
		send_to_char(ch, "Usage: mkaff <market> <affection> <modif> <duration> <value>\r\n");
		return;
	}
	...
	if (!*arg)
	{
		send_to_char(ch, "Available affections are:\r\n");
	...
	{
		send_to_char(ch, "Unknown affection.\r\nAvailable affections are:\r\n");
	...
	if (!*arg)
	{
		send_to_char(ch, "Available modifiers are:\r\n");
	...
	{
		send_to_char(ch, "Unknown modifier.\r\nAvailable modifiers are:\r\n");
	...
	if (!*arg)
	{
		send_to_char(ch, "Value must be a number from -2.0 to 2.0\r\n");
		return;
	}
...
ACMD(do_goodinfo)
{
	...
	if (!*arg || !is_number(arg))
	{
		send_to_char(ch, "Usage: ginfo <good vnum>\r\n");
		return;
	}
