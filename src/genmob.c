...
int save_mobiles(zone_rnum rznum)
{
  ...
  if ((mobfd = fopen(mobfname, "w")) == NULL) {
    nmudlog(SYS_LOG, LVL_IMPL, TRUE, "SYSERR: GenOLC cannot open mob file for writing");
...
int write_mobile_record(mob_vnum mvnum, struct char_data *mob, FILE *fd)
{
  ...
  if (write_mobile_espec(mvnum, mob, fd) < 0)
    nmudlog(SYS_LOG, LVL_IMPL, TRUE, "SYSERR: GenOLC had an error writing E-specs for mobile %d", mvnum);
  ...
  if (write_mobile_mobprog(mvnum, mob, fd) < 0)
    nmudlog(SYS_LOG, LVL_IMPL, TRUE, "SYSERR: GenOLC had an error writing MobProgs for mobile %d", mvnum);
...
void check_mobile_string(mob_vnum i, char **string, const char *desc)
{
  if (*string == NULL || **string == '\0') {
    ...
    nmudlog(SYS_LOG, LVL_IMPL, TRUE, "GenOLC error with mob %d that has an invalid %s", i, desc);
