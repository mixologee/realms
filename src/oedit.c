...
ACMD(do_oasis_oedit)
{
  ...
  if ((real_num = real_object((long int) number)) != NOTHING)
...
void oedit_disp_val1_menu(struct descriptor_data *d)
{
  ...
  case ITEM_FURNITURE:
    ...
    break;
  case ITEM_CRYGATE:
    write_to_output(d, "Use the following list for the next 3 questions.\r\n");
    write_to_output(d, " &g0&w) NULL - &RDO NOT SET\r\n"
                       " &g1&w) Octagon       &g2&w) Triangle\n"
                       " &g3&w) Circle        &g4&w) Pentagon\n"
                       " &g5&w) Line          &g6&w) Sun\n"
                       " &g7&w) Moon          &g8&w) Hand\n"
                       " &g9&w) Arrow&n\r\n\r\n");
    write_to_output(d, "Symbol1: \r\n");
...
void oedit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case OEDIT_MAIN_MENU:
    ...
    case 'a':
    case 'A':
      write_to_output(d, "Enter condition : ");
      OLC_MODE(d) = OEDIT_CONDITION;
      break;
    case 'b':
    case 'B':
  ...
  case OEDIT_CONDITION:
    GET_OBJ_COND(OLC_OBJ(d)) = LIMIT(atoi(arg), 0, MAX_OBJ_RENT);
    break;
  ...
  case OEDIT_COPY:
    if ((number = real_object(((long int) atoi(arg)))) != NOTHING) {
