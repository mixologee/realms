...
long int top_of_builds=5000000;
/* ================================================================= */
...
void list_building_to_char(BUILDING_DATA *bldlist, CHAR_DATA *ch)
{
	...
	if (ROOM_FLAGGED(ch->in_room, ROOM_BUILDING_WORKS))
		send_to_char(ch, "There are building works here.\r\n");
}
...
void char_to_building(CHAR_DATA *ch, BUILDING_DATA *bld, int room)
{
	...
	ch->in_building	= bld;
	char_to_room(ch, bld->rooms[room_num]->number);
...
bool can_enter_bld(CHAR_DATA *ch, BUILDING_DATA *bld, bool bCanAuthCheck)
{
	...
	if (bld->can_enter == BLD_ENT_FREE)
	{
		if (bCanAuthCheck)
		{
			send_to_char(ch, "You don't need to set/revoke authorization for free entrance buildings.\r\n");
			return (FALSE);
		}
		return (TRUE);
	}
...
void enter_building(CHAR_DATA *ch, BUILDING_DATA *bld)
{
	...
	{
		send_to_char(ch, "You cannot enter in a ruined building.\r\n");
		return;
	}
	...
	if (bld->type->vnum != BLD_STABLE)
	{
		if (RIDING(ch))
		{
			send_to_char(ch, "You cannot enter a building while mounted.\r\n");
			return;
		}
		if (WAGONER(ch))
		{
			send_to_char(ch, "You cannot enter a building while driving a vehicle.\r\n");
			return;
		}
	}
	...
	char_from_room(ch);
	log("about to call char_to_building");
...
void auth_enter(BUILDING_DATA *bld, CHAR_DATA *ch, CHAR_DATA *vict, int mode)
{
	...
	else
	{
		...
		{
			send_to_char(ch, "You cannot revoke authorization to someone that's currently inside.\r\n");
			return;
		}
...
BUILDING_DATA *find_char_owned_building(CHAR_DATA *ch)
{
	...
	if (!ch || !IN_ROOM(ch) || !world[IN_ROOM(ch)].buildings)
		return (NULL);

	for (bld = world[IN_ROOM(ch)].buildings; bld; bld = bld->next_in_room)
	{
...
EXIT_DATA *find_building_door(CHAR_DATA *ch, char *doorname, char *building)
{
	...
	if (!ch || !ch->in_room ||  world[IN_ROOM(ch)].buildings)
		return (NULL);

	if (!(bld = find_building_in_room_by_name(pRoom, building)))
		return (NULL);
...
void building_to_room(BUILDING_DATA *bld, ROOM_DATA *pRoom)
{
	...
	if (pRoom)
	{
                log("building to room - room info passed");
		...
		if (IS_WILD(pRoom))
		{
			...
			bld->coord->y	= GET_RY(pRoom->number);
			bld->coord->x	= GET_RX(pRoom->number);
			
			SET_BIT_AR(ROOM_FLAGS(pRoom->number), ROOM_WILD_STATIC);
		}
		else
	...
	{
                log("building to room - zone");
		if (!(room = &world[real_room(bld->vnum_room)]))
		{
			log("SYSERR: Cannot place building to room %ld.", bld->vnum_room);
			return;
		}
		bld->coord	= NULL;
	}
	...
	{
		log("building to room - wilderness");
...
bool create_building_rooms(BUILDING_DATA *bld)
{
	...
	char fname[128], buf2[MAX_INPUT_LENGTH];
	char letter;//, *flags;
	int room, door;
  	int t[10], i, retval;
  	char line[READ_SIZE], flags2[128], flags3[128];
  	char flags4[128], buf[128];
        char *flags;

	log("Calling create_building_room");
	...
	for (room = 0; room < bld->type->size; room++)
	{
		...
		bld->rooms[room]->number = room + BUILDZONE;
		bld->rooms[room]->zone = BUILDZONE;
	...
	for ( ;; )
	{
		...
		if (letter == '*')
		{
			log("skip comment while reading building file.");
			fread_to_eol(fp);
			continue;
		}
		...
		room = fread_number(fp);
                log("room number read in is %ld", room);
...
/*		scanf(line, " %d %s %s %s %s %d ", t, flags, flags2, flags3, flags4, t + 2);
		bld->rooms[room]->room_flags[0] = asciiflag_conv(flags);
    		bld->rooms[room]->room_flags[1] = asciiflag_conv(flags2);
    		bld->rooms[room]->room_flags[2] = asciiflag_conv(flags3);
    		bld->rooms[room]->room_flags[3] = asciiflag_conv(flags4);
*/
                flags                                                   = fread_word(fp);
//		bld->rooms[room]->room_flags	= asciiflag_conv(flags);

                bld->rooms[room]->room_flags[0] = asciiflag_conv(flags);
                bld->rooms[room]->room_flags[1] = 0;
                bld->rooms[room]->room_flags[2] = 0;
                bld->rooms[room]->room_flags[3] = 0;
		...
		for ( ;; )
		{
			...
			{
			case 'D':
                                log("Found door for room# %ld in building# %d", room, bld->vnum);
				...
				else
				{
					...
					pexit->general_description	= fread_string_nospace(fp);
					...
					pexit->to_room		= bld->rooms[pexit->vnum]->number;
...
BUILDING_DATA *setup_new_building(BUILDING_TYPE *bType, ROOM_DATA *pRoom, long owner_id, char owner_type)
{
	...
	if (!bType || !pRoom)
	{
       ...
       log("Entering setup_new_building buildings.c line 1127");
...
//	{
                log("owner type = bld_own_char  buildings.c line 1151");
...
/*	}
	...
	{
                log("clan building buildings.c line 1158");
...
*/
	...
	if (bld->type->vnum == BLD_STORE || bld->type->vnum == BLD_STABLE)
        ...
        log("Adding buildings stats   buildings.c line 1187");
	/* set up defense pts */
	bld->max_val.defense	= rand_number(bType->range_min.defense, bType->range_max.defense);
	bld->curr_val.defense	= bld->max_val.defense;
	/* set up health pts */
	bld->max_val.health		= rand_number(bType->range_min.health, bType->range_max.health);
	...
	if (bType->range_max.mob_attack)
	{
		...
		bld->max_val.mob_attack		= rand_number(bType->range_min.mob_attack, bType->range_max.mob_attack);
	...
	{
		bld->max_val.npc_attack		= rand_number(bType->range_min.npc_attack, bType->range_max.npc_attack);
		bld->curr_val.npc_attack	= bld->max_val.npc_attack;
	}

//	if (ROOM_FLAGGED(pRoom->number, ROOM_WILDERNESS))
//	{
//		bld->coord->x = world[pRoom->number].x;
//		bld->coord->y = world[pRoom->number].y;
//	}

	// execute building commands (load objs & mobs)
	log("Executing building commands buildings.c line 1217");
	exec_bld_cmds(bld);
	...
	log("Calling building_to_room   buildings.c 1226");
	building_to_room(bld, pRoom);

	/* exit building --> room */
        log("Making Exits buildings.c 1229");
	pexit = make_exit(bld->rooms[bld->type->entrance], NULL, SOUTH);
	pexit->to_room = bld->in_room->number;
	...
	// forced save
	log("Forcing savebuildiongs   buildings.c 1239");
	SaveBuildings();
...
int build_works_start(BUILDING_TYPE *bType, ROOM_DATA *pRoom, long owner_id, char owner_type)
{
	...
	if (IS_WILD(pRoom))
	{
		...
		pWorks->coord->y	= GET_RY(pRoom->number);
		pWorks->coord->x	= GET_RX(pRoom->number);
		SET_BIT_AR(ROOM_FLAGS(pRoom->number), ROOM_BUILDING_WORKS | ROOM_WILD_STATIC);
	}
	else
	{
		pWorks->in_room		= pRoom;
		SET_BIT_AR(ROOM_FLAGS(pRoom->number), ROOM_BUILDING_WORKS);
	}
	...
	for (pWorks = first_build_work; pWorks; pWorks = pWorks_next)
	{
		...
		if (--pWorks->timer <= 0)
		{
			...
			if (++pWorks->phase > BWKS_FINAL)
			{
				...
				REMOVE_BIT_AR(ROOM_FLAGS(pWorks->in_room->number), ROOM_BUILDING_WORKS);
...
void build_works_list(CHAR_DATA *ch)
{
	...
	if (!first_build_work)
	{
		send_to_char(ch, "Currently, there are no building works in progress.\r\n");
		return;
	}

	send_to_char(ch, "List of build works currently in progress:\r\n");
...
void SaveWorksList(void)
{
	...
	for (pWorks = first_build_work; pWorks; pWorks = pWorks->next)
	{
		fprintf(fp, "Work %d %ld %ld %d %ld %ld %d ",
			pWorks->num,
			pWorks->owner_type, pWorks->owner_id, (pWorks->authorized ? 1 : 0),
			pWorks->phase, pWorks->timer, pWorks->type);
		if (pWorks->coord)
			fprintf(fp, "C %ld %ld", pWorks->coord->y, pWorks->coord->x);
		else
			fprintf(fp, "R %ld", pWorks->in_room->number);
		fprintf(fp, "\n");
	}
...
void create_building(CHAR_DATA *ch, char *arg, int clan_num)
{
	...
	if (!num_of_bld_templ)
	{
		send_to_char(ch, "No building can be built right now.\r\n");
		return;
	}

	if (!*arg)
	{
		send_to_char(ch, "Usage: build <building type>\r\n");
		show_valid_list(ch);
		return;
	}
	...
	{
		send_to_char(ch, "Unknown building type '%s'.\r\n", arg);
		show_valid_list(ch);
		return;
	}
...
//	{
//		send_to_char(ch, "You cannot build in this room.\r\n");
//		return;
//	}
...
/*	}
	...
	{
		struct clan_type *pClan = NULL;
		for (pClan = clan_info; pClan && pClan->number != GET_CLAN(ch); pClan = pClan->next);

		if ( !pClan )
		{
			send_to_char(ch, "Ooopss.. clan error: contact an immortal.\r\n");
			log("SYSERR: create_building() - invalid clan number %d.", clan_num);
			return;
		}

		owner_id	= GET_CLAN(ch);
...
*/
	...
	if (GET_LEVEL(ch) > LVL_IMMORT)
		setup_new_building(building_template[vnum], room, owner_id, owner_type);
	else
		build_works_start(building_template[vnum], room, owner_id, owner_type);
}
...
void do_stat_building(CHAR_DATA *ch, BUILDING_DATA *bld)
{
	...
	if (bld->coord)
		ch_printf(ch, "Location   : %d %d\r\n", GET_RY(bld->in_room->number), GET_RX(bld->in_room->number));
	else
		ch_printf(ch, "Location   : room %d\r\n", bld->vnum_room);

	sprintbit(bld->flags, bld_flags_descr, bflags, sizeof(bflags));
...
ACMD(do_bldlist)
{
	...
	if (!building_loaded)
	{
		send_to_char(ch, "No buildings at all.\r\n");
		return;
	}

	send_to_char(ch, "List of building in world:\r\n");
	for (iHash = 0; iHash < BLD_HASH; iHash++)
	{
		for (bld = building_list[iHash]; bld; bld = bld->next)
		{
			if (IS_WILD(bld->in_room))
				sprintf(sbaf, "Wild [%d %d]", GET_RY(bld->in_room->number), GET_RX(bld->in_room->number));
			else
...
void fwrite_building_rooms(BUILDING_DATA *bld, FILE *fp)
{
	ROOM_DATA *room;
	EXIT_DATA *pexit;
	char dflags[128], buf[MAX_INPUT_LENGTH], buf1[MAX_INPUT_LENGTH], buf2[MAX_INPUT_LENGTH];
	int num;
	...
	for (num = 0; num < bld->size; num++)
	{
		...
		//		continue;
			...
			{
				strcpy(buf, pexit->general_description);
				strip_cr(buf);
			}
			else
				*buf = '\0';
			...
			/* sanity check */
			if (EXIT_FLAGS(pexit) && pexit->exit_info != EX_ISDOOR)
				SET_BIT(pexit->exit_info, EX_ISDOOR);
			...
			fprintf(fp,
				...
				pexit->key, ( pexit->to_room != NULL ? pexit->vnum : NOWHERE ) );
		}
	...
	for ( ; ; )
	{
		...
		case '#':
			...
			fread_to_eol( fp );
                         ...
                         if ((obj = objsave_parse_objects(fp)) == NULL)
//			if ((obj = fread_one_obj(fp, &location)) == NULL)
				continue;
			
			obj_to_room(obj, bld->rooms[roomnum]->number);
			...
			for (j = MAX_BAG_ROWS - 1; j > -location; j--)
			{
				if (cont_row[j])	/* No container, back to inventory. */
				{
					for (; cont_row[j]; cont_row[j] = obj2)
					{
						...
						obj_to_room( cont_row[j], bld->rooms[roomnum]->number );
					}
					cont_row[j] = NULL;
				}
			}
...
void fread_building_rooms(BUILDING_DATA *bld, FILE *fp)
{
	...
	int room, door;
        ...
        log("creating building rooms array");
...
CHAR_DATA *get_bld_defmob(BUILDING_DATA *bld)
{
	...
	else			num = rand_number(0, nlast);		// two or more mobs

	if (!(mob = read_mobile(bld->vmob[num], VIRTUAL)))
	{
		log("SYSERR: get_bld_defmob() - invalid mob vnum %ld.", bld->vmob[num]);
		return (NULL);
	}
...
void damage_people_inside(BUILDING_DATA *bld)
{
	...
	for (tch = bld->people; tch; tch = tch_next)
	{
			...
			roll(GET_DEX(tch) - 4))
		{
			...
			char_from_room(tch);
			char_to_room(tch, bld->in_room->number);
		...
		send_to_char(tch, "You are hit by falling rubbles.\r\n");

		// char suffer massive damage.. :-(((
		damage(tch, tch, rand_number(50,100) * (bld->type->vnum + 1), -1);
	}
}
...
void hit_building(CHAR_DATA *ch, BUILDING_DATA *bld)
{
	...
	{
		send_to_char(ch, "You cannot damage a building with your bare hands.\r\n");
		return;
	}

	if (GET_OBJ_TYPE(weapon) != ITEM_WEAPON)
	{
		send_to_char(ch, "Using a weapon would surely help.\r\n");
		return;
	}
	...
	{
		// message to attacker
		act("Your attack DESTROYS $b.", FALSE, ch, NULL, bld, TO_CHAR);
		// message to room
		act("$n DESTROYS $b with $s attack.", FALSE, ch, NULL, bld, TO_ROOM);
                xp = 100000*atoi(bld->type);
                gain_exp(ch, xp);
		return;
	}
...
bool attack_building(CHAR_DATA *ch, char *arg)
{
	...
	if (!(bld = find_building_in_room_by_name(&world[ch->in_room], arg)))
        {
	//	ch_printf(ch, "No building like that here.\r\n");
		return (FALSE);
	}
...
ACMD(do_authorize)
{
	...
	{
		send_to_char(ch, "You can use this command only when inside a building or aboard a ship.\r\n");
		return;
	}
	...
	if (!str_cmp(arg, "list"))
	{
		if (pBld)
		{
			...
			{
				send_to_char(ch, "You have no rights, go away.\r\n");
				return;
			}
			bAuthList = pBld->auth;
		}
		else if (pShip)
		{
			...
			{
				send_to_char(ch, "You must be the captain or the owner to authorize people.\r\n");
				return;
			}
		...
		if (!bAuthList)
		{
			send_to_char(ch, "No one is authorized.\r\n");
			return;
		}
	...
	if (pBld)
	{
		...
		{
			ch_printf(ch, "No one called '%s' is in the game at the moment.\r\n", arg);
			return;
		}
		...
		{
			send_to_char(ch, "You cannot authorize someone you don't know.\r\n");
			return;
		}
	...
	else if (pShip)
	{
		...
		{
			send_to_char(ch, "You can authorize people only when ship is at one port.\r\n");
			return;
		}
