...
struct mob_party *party_list = NULL;
int top_of_partylist =0;

struct mail_data *mail_list;
long int top_of_maillist = 0;

SURVEY_DATA     *survey_table   = NULL;         /* survey descriptions                  */
ROOM_AFFECT     *raff_list      = NULL;         /* global list of room affections       */

TERRAIN_DATA    *terrain_type[MAX_SECT_TYPES];
int    Sunlight;
...
void clear_party(struct mob_party *mp);
...
extern int top_news_version;
extern int top_gnews_version;
extern int disable_wilderness;


void PopulateWild(void);
void load_bank(void);
void  LoadMapImage(void);
void  InterperateMapArray(void);
void  LinkMapRooms(void);
...
void load_news(void)
{
  FILE *f;
    ...
    if (!(f = fopen("../news/news.dat", "r"))){
         log("../news/news.dat");
         exit(1);
        }
        fscanf(f," %d ",&top_news_version);
        fclose(f);
}

void load_gnews(void)
{
  FILE *fp;
  
   log("Opening God News file index.");
...
//log("Wilderness Mapping disabled -- Quick boot");
  if(!disable_wilderness) {
    log("Building Wilderness");
    BuildWild();
  }
...
void boot_db(void)
{
  ...
  log("Loading news and gnews files");
  load_news();
  load_gnews();
  load_help();
...
time_info = *mud_time_passed(time(0), beginning_of_time);
  ...
  log("   Current Gametime: %dH %dD %dM %ldY.", time_info.hours,
...
void parse_simple_mob(FILE *mob_f, int i, int nr)
{
  ...
  GET_MAX_MANA(mob_proto + i) = GET_LEVEL(mob_proto + i) * 10;
  GET_MAX_MOVE(mob_proto + i) = GET_LEVEL(mob_proto + i) * 15;
...
void interpret_espec(const char *keyword, const char *value, int i, int nr)
{
  ...
  CASE("Clss") {
   RANGE(1,21);
   mob_proto[i].mob_specials.class = num_arg;
 }  
...
//    fprintf(stderr,"line 2004 db.c flags = %d\r\n", AFF_FLAGS(mob_proto + i)[taeller]);
    }
  } else {
...
char *parse_object(FILE *obj_f, int nr)
{
  ...
  /* Setting bound value to NOBODY regardless, as obj protos should never bind to a player, done in rent files only */
...
struct obj_data *tweak_object(struct obj_data *obj)
{
  int  j, t;

  if (OBJ_FLAGGED(obj, ITEM_NOTWEAK))
     return(obj);
 ...
 for (j = 0; j < MAX_OBJ_AFFECT; j++)
 {
   t = rand_number(0, 1000);

   if(IS_SET_AR(GET_OBJ_EXTRA(obj), ITEM_NOBADTWEAK) && t < 50)
   {
     t += 500;
   }


   if (obj->affected[j].location == 0)
      continue;

   if (obj->affected[j].location >= 12 && obj->affected[j].location <= 14)
   {
      if (t == 0)
      {
        obj->affected[j].modifier = rand_number(obj->affected[j].modifier / 4 , obj->affected[j].modifier);
        continue;
      }
      if (t > 0 && t <= 5)
      {
        obj->affected[j].modifier = rand_number(obj->affected[j].modifier / 2 , obj->affected[j].modifier);
        continue;
      }
      if (t > 6 && t <= 49)
      {
      ...
      if (t > 49 && t <= 500)
      {
        obj->affected[j].modifier = rand_number(obj->affected[j].modifier / 2 , obj->affected[j].modifier * 2);
        continue;
      }
       if (t > 500 && t <= 950)
      {
        obj->affected[j].modifier = rand_number(obj->affected[j].modifier, obj->affected[j].modifier * 2);
        continue;
      }
       if (t > 950 && t <= 995)
      {
      ...
      if (t > 995 && t <= 999)
      {
        obj->affected[j].modifier = rand_number(obj->affected[j].modifier, obj->affected[j].modifier * 3);
        continue;
      }
      if (t == 1000)
      {
        obj->affected[j].modifier = rand_number(obj->affected[j].modifier, obj->affected[j].modifier * 4);
        continue;
      }
   ...
   else
   {
      if (t < 5)
      {
        obj->affected[j].modifier = obj->affected[j].modifier - 1;
        continue;
      }
      if (t > 4 && t < 10)
      {
      ...
      if (t > 9 && t < 300)
      {
        obj->affected[j].modifier = rand_number(obj->affected[j].modifier - 1, obj->affected[j].modifier);
        continue;
      }
      if (t > 299 && t <= 500)
      {
        obj->affected[j].modifier = rand_number(obj->affected[j].modifier - 1, obj->affected[j].modifier + 1);
        continue;
      }
      if (t > 500 && t < 700)
      {
      ...
      if (t > 699 && t < 800)
      {
        obj->affected[j].modifier = rand_number(obj->affected[j].modifier + 1, obj->affected[j].modifier + 2);
        continue;
      }
      if (t > 799 && t < 1000)
      {
        obj->affected[j].modifier = obj->affected[j].modifier + 2;
        continue;
      }
      if (t == 1000)
      {
        obj->affected[j].modifier = obj->affected[j].modifier + 3;
        continue;
      }
   }
 }
...
void load_zones(FILE *fl, char *zonename)
{
  ...
  int zone_fix = FALSE;
  char t1[80], t2[80];
  char zbuf1[MSL], zbuf2[MSL], zbuf3[MSL],zbuf4[MSL];
...
/* COMMENTING OUT OLD ZEDIT TO INSERT NEW FUNCTIONS */
...
//      zone_fix = TRUE;
   ...
   /* Instead of the standard 4 items in a zone file, look for our new 10.. if not found, look for OLD 4 */
      ...
      &Z.reset_mode, zbuf1, zbuf2, zbuf3, zbuf4, &Z.min_level, &Z.max_level) != 10)
  {
    if (sscanf(buf, " %d %d %d %d ", &Z.bot, &Z.top, &Z.lifespan, &Z.reset_mode) != 4) {
     ...
     /* Only found 4 vals, so set the defaults of the other ones */
     Z.min_level = -1;
     Z.max_level = -1;
  }
  else
...
/* create an object, and add it to the object list */
struct mob_party *create_mp(void)
{
  struct mob_party *mp;

  CREATE(mp, struct mob_party, 1);
  clear_party(mp);
  mp->next = party_list;
  party_list = mp;
...
struct obj_data *read_plr_object(obj_vnum nr, int type) /* and obj_rnum */
{
  struct obj_data *obj;
  obj_rnum i = type == VIRTUAL ? real_object(nr) : nr;
...
//      mudlog(CMP, LVL_IMPL, FALSE, "Auto zone reset: %s (Zone %ld)",
      nmudlog(ZONE_LOG, LVL_GOD, TRUE, "Auto zone reset: %s (Zone %d)", 
	zone_table[update_u->zone_to_reset].name, zone_table[update_u->zone_to_reset].number);
//      mudlog(ZRT, LVL_IMPL, FALSE, "Auto zone reset: %s (Zone %d)",
//          zone_table[update_u->zone_to_reset].name, zone_table[update_u->zone_to_reset].number);
      /* dequeue */
...
void log_zone_error(zone_rnum zone, int cmd_no, const char *message)
{
  nmudlog(INFO_LOG, LVL_GOD, TRUE, "SYSERR: zone file: %s", message);
  nmudlog(INFO_LOG, LVL_GOD, TRUE, "SYSERR: ..offending command: '%c' command in zone #%d, line %d", 
  ...
  for (cmd_no = 0; ZCMD.command != 'S'; cmd_no++) {
    ...
    case 'M':                  /* read a mobile */
      if (mob_index[ZCMD.arg1].number < ZCMD.arg2) {
        ...
        load_mtrigger(mob);        
        if (OUTSIDE(mob) && MOB_FLAGGED(mob, MOB_DAY_ONLY) && Sunlight == SUN_DARK) {
	  ...
	  mob->carrying = NULL;
        ...
        if (OUTSIDE(mob) && MOB_FLAGGED(mob, MOB_NIGHT_ONLY) && Sunlight == SUN_LIGHT) {
          for (i = 0; i < NUM_WEARS; i++) {
            if (GET_EQ(mob, i)) 
              unequip_char(mob, i);
          }
...
void clear_party(struct mob_party *mp)
{
  memset((char *) mp, 0, sizeof(struct mob_party));
...
void init_char(struct char_data *ch)
{
 ...
 for(x=0;x < MAX_STOCK;x++)
    SET_SHARES(mob_proto + i, x, 0);

  for (i = 0; i < 3; i++)
    GET_COND(ch, i) = (GET_LEVEL(ch) == LVL_IMPL ? -1 : 24);

  GET_LOADROOM(ch) = 5800;
}
...
int check_object(struct obj_data *obj)
{
  ...
  if (GET_OBJ_COND(obj) > 1500 && (error = TRUE))
    log("SYSERR: Object #%d (%s) should be gone with condition (%d).",
	GET_OBJ_VNUM(obj), obj->short_description, GET_OBJ_COND(obj));
...
int check_bitvector_names(bitvector_t bits, size_t namecount, const char *whatami, const char *whatbits)
{
  ...
  if (bits <= (~(bitvector_t)0 >> (sizeof(bitvector_t) * 4 - namecount)))
    return (FALSE);

  for (flagnum = namecount; flagnum < sizeof(bitvector_t) * 4; flagnum++)
    if ((1 << flagnum) & bits) {
...
extern int password_retrieval_allowed;
extern int mail_objects_allowed;
extern int mail_gold_allowed;
extern int mail_stamp_cost;
extern int mail_object_cost;
extern int mail_min_level;
extern int mail_min_free_level;
extern int mail_allow_drafts;
extern int mail_draft_timeout;
extern int mail_safe_mailing;
extern int mail_min_mail_anywhere;
extern int mail_min_send_to_all;
...
void load_default_config( void )
{
  ...
  CONFIG_PWD_RETRIEVAL          = password_retrieval_allowed;
  ...
  /* Mudmail options */
  CONFIG_CAN_MAIL               = can_mail;
  CONFIG_CAN_MAIL_OBJ           = mail_objects_allowed;
  CONFIG_CAN_MAIL_GOLD          = mail_gold_allowed;
  CONFIG_STAMP_COST             = mail_stamp_cost;
  CONFIG_OBJECT_COST            = mail_object_cost;
  CONFIG_MIN_MAIL_LEVEL         = mail_min_level;
  CONFIG_FREE_MAIL_LEVEL        = mail_min_free_level;
  CONFIG_DRAFTS_ALLOWED         = mail_allow_drafts;
  CONFIG_DRAFT_TIMEOUT          = mail_draft_timeout;
  CONFIG_SAFE_MAILING           = mail_safe_mailing;
  CONFIG_MIN_MAIL_ANYWHERE      = mail_min_mail_anywhere;
  CONFIG_MIN_SEND_TO_ALL        = mail_min_send_to_all;
  ...
  while (get_line(fl, line)) {
    ...
    switch (LOWER(*tag)) {
      ...
      case 'p':
        ...
        else if (!str_cmp(tag, "pwd_retrieval_allowed"))
          CONFIG_PWD_RETRIEVAL = num;
        break;
      ...
      case 's':
        ...
        else if (!str_cmp(tag, "safe_mailing"))
          CONFIG_SAFE_MAILING  = num;
        break;
...
ROOM_DATA *new_room( void )
{
        ...
        pRoom->room_flags[2] 		= 0;
        pRoom->room_flags[3] 		= 0;
...
void setup_wild_exit(FILE *fl, ROOM_DATA *pRoom, int door)
{
        ROOM_DATA *wRoom;
        EXIT_DATA *pexit;
        char flags[128], buf2[MAX_STRING_LENGTH];
        char *ln;
        int x1, x2, x3;
        ...
        pexit->general_description      = fread_string( fl, buf2 );
        ...
        if ( SECT(wRoom->number) != SECT_WILD_EXIT )
        {
                put_sect( wRoom->coord->y, wRoom->coord->x, SECT_WILD_EXIT, TRUE );
                world[wRoom->number].sector_type = SECT_WILD_EXIT;
//                SECT(wRoom->number)     = SECT_WILD_EXIT;
        }
        setup_wild_static( wRoom, pRoom );
        pexit->to_room          = find_wild_room( wRoom, door, FALSE );
        pexit->coord->y         = world[pexit->to_room].coord->y;
        pexit->coord->x         = world[pexit->to_room].coord->x;
}
...
void make_maze(zone_vnum zone)
{
        ...
        for (test = 0; test < 400; test++)
        {
                ...
                REMOVE_BIT_AR(ROOM_FLAGS(pRoom->number), ROOM_NOTRACK);
        }
        ...
        for (num = 0; num < 400; num++)
        {
                ...
                pNextRoom = &world[real_room( next_room )];
 ...
 *                                   * the maze
 *                                                    */
                SET_BIT_AR(ROOM_FLAGS(pRoom->number), ROOM_NOTRACK);
                REMOVE_BIT_AR(ROOM_FLAGS(pRoom->number), ROOM_BFS_MARK);
        }
}
